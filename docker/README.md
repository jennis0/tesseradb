...
The image holds the `tessera` binary on a distroless Debian base: glibc and CA certificates, with no shell and no package manager. It runs as uid 65532, and the root filesystem can be mounted read-only. The server tunes glibc's allocator, which is why the base is glibc rather than musl.
