...
**`points.parquet` (five columns) is the realistic one**; **`points-wide.parquet` (eighteen) is
...
The wide fixture is not five columns padded to eighteen. Every column earns its place by
...
Outputs, under `data/scaled/attrs/`:
    ...
    points-wide.parquet       the same, + all eighteen
...
#
# Generated by `probes/build_attributes.py`. Eighteen columns, each exercising something the
...
#
# Residency, which `tessera build` also reports: ~57 B/row over the 12 B fixed row.
...
def main():
    ...
    log(
        ...
        f"{points_wide.stat().st_size / 1e6:.1f} MB (18 columns)"
    )
