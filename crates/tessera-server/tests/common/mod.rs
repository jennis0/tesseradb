...
pub async fn mount_server(engine: Engine, max_k: usize, compute_gate: ComputeGate) -> TestServer {
    mount_server_with_cors(engine, max_k, compute_gate, Vec::new()).await
}
...
pub async fn spawn_server_with_cors(
    ...
    let engine = Engine::open(bundle_root, cache_dir, wal_path, Passthrough::new(), config)
        .expect("engine should open against a freshly built bundle");
    mount_server_with_cors(engine, max_k, generous_test_gate(), dev_cors_origins).await
}

/// The body of [`mount_server`], with the CORS origin list as a parameter.
pub async fn mount_server_with_cors(
    engine: Engine,
    max_k: usize,
    compute_gate: ComputeGate,
    dev_cors_origins: Vec<String>,
