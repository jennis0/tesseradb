...
/// **A declared scalar column an input lacks is absent in every row of that input**, never
/// dropped and never a held zero (`ingest.md` §6.3).
///
/// Dropping it would shift every later scalar up a position, so the merged segment would carry
/// every value under the wrong column's name — right count, right types, wrong data, and no error
/// anywhere. The inputs here are written with no scalars while the merge declares one, which is
/// the shape a column declared at a running service presents to a merge of segments written
/// before it: the merged segment carries the column at its placeholder, and its presence bitmap
/// leaves every one of those rows out, so a reader takes the absence and not the zero.
///
/// **Mutation:** restore the `filter_map` in `gather_scalars` and the writer refuses a short row;
/// drop the schema check in the merge's presence pass and every row reads as carrying a zero.
#[test]
fn a_missing_scalar_column_is_absent_in_every_row_rather_than_shifting_the_rest() {
    let dir = tempfile::TempDir::new().unwrap();
    build_bundle(dir.path(), 10);
    ...
    assert_eq!(cols.tessera_id().len(), 10, "the fixed columns keep every row");
}
