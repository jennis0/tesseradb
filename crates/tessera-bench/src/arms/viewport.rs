...
//!
//! `docs/design-memos/2026-07-30-tail-attribution.md` showed that a uniform-random p99 mostly
        ...
        for &coverage_target in coverages {
            ...
            for &mode in &modes {
                ...
                for &k in ks {
                    ...
                    let warm =
                        engine.viewport(&session, &slice_id, plan[0].0, plan[0].1, k, None)?;
                    ...
                    for (index, (vz, bbox)) in plan.iter().enumerate() {
                        ...
                        let samples = crate::metrics::repeat(ctx.repeat, || {
                            let out = engine
                                .viewport(&session, &slice_id, *vz, *bbox, k, None)
                                .expect("viewport");
                            let t = out.timings;
                            last = Some(t);
                            out
...
fn density_deciles(
    ...
    for (z, bbox) in candidates {
        let out = engine.viewport(session, slice, z, bbox, 0, None)?;
