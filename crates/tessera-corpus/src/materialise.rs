...
#
# One points file carries identity, geometry and every declared column, so `[corpus]` and the view
# name one file; the exploded `(entity_id, term_id)` relation is the other. Both are named
# relative to this document, which is written beside them (`configuration.md` §3).
#
