...
/// **A dependent layer's column is the membership its artifacts are served over.** A label
/// published with the members it describes names itself on those points; one published with **no**
/// members is served over its cluster's membership and names its cluster's points (owner ruling H,
/// 2026-09-18); and a label whose cluster this response does not hold is absent from the frame and
/// so from the column.
#[test]
fn a_dependent_layer_resolves_over_the_membership_it_is_served_over() {
    ...
    // **The label that declared no members of its own is the label of `b1`** (the H ruling): it is
    // placed where `b1` is placed, so it is a candidate in `b1`'s tiles and names `b1`'s points in
    // the column exactly as a label published over those members would.
    assert_eq!(
