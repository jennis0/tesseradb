...
fn response_tile_order_and_point_concatenation_follow_tiles_for_bbox_not_morton_order() {
    ...
    let session = engine.authorise(&full_coverage_credential()).unwrap();
    let out = engine
        .viewport(&session, "s0", ZOOM, bbox, K, None)
        .unwrap();
    ...
    for tile in &tiles {
        // Full-coverage session: every row in the range is visible, so the placeholder sampler's
        // "first k visible rows in row order" is the first k rows of the range itself.
        let range = tessera_store::tile_ranges(segment, tile);
        if range.is_empty() {
            continue;
        }
        expected_prefixes.push(tile.prefix);
        for row in range.clone().take(K) {
            expected_points.push(tessera_ids[row as usize]);
        }
    }
