...
///
/// **The composed mask is deliberately not a field.** A filtered request holds two at once — the
/// pre-filter mask the region and `member_of` resolvers close over, and the narrowed one the sweep
/// reads — so which of them a callee is given stays visible at the call site.
