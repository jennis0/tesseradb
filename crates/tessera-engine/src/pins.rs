//! The pin manager: Arc plus a drain list, with one ordering rule (I11, lifecycle §2.1–§2.3).
//!
//! A pin fixes **row-space geometry** — the prefix, the segment-set version, the permutation and
//! the columns those name — and **never authorisation state**. That sentence is the whole module.
//! Everything below exists either to keep a superseded geometry resolvable for a bounded while
//! ([`PinManager::retire`], [`PinManager::reclaim`]) or to make it impossible for a pinned request
//! to reach the overlay, buffer or `overlay_version` that geometry was swapped away from
//! ([`PinnedGeometry`]).
//!
//! **Why a drain list at all.** Before Task 4 a pin was an equality check against the live
//! generation, which is correct only because there has never been more than one: nothing in
//! Phase 1 or stage 2.1 moves `segments_version`. Stage 2.2's flush and 2.3's compaction both
//! supersede a generation while requests are in flight, and at that moment an equality check
//! expires every outstanding pin — the pannable map's whole point being that a drill-down lands on
//! the geometry the viewport was drawn from.
//!
...
//!    check. Every request calls it, at the branch's 48-way admission concurrency; a per-request
//!    global mutex here is precisely the contention class the concurrency workstream's F4 work
//!    removed from this path. The cold path is `#[cold]` and separated into its own function so
//!    that this is visible rather than argued, and [`PinManager::lock_drain`] is the **single**
//!    place this module locks, counting every acquisition so the property is testable.
...
//! 4. **What actually bounds retention is the TTL and [`DRAIN_DEPTH_MAX`], not §2.2's per-session
//!    cap.** See [`PinManager::pins_per_session_max`] — the cap is worth having and the spec
//!    requires it, but it is a politeness limit and this module says so rather than repeating a
//!    page-cache argument it cannot deliver.
...
/// Drain depth above which an operator should alarm (lifecycle §2.2, and the plan's Task 4).
///
/// Depth 1 is ordinary: a compaction publishes, the superseded prefix drains, and one entry sits
/// there until its TTL. Depth 2 means a *second* geometry was published before the first drained,
/// which is the shape the bound exists to catch — at the probes' **measured** 47.02 GB bundle with
/// a 22.5 GB viewport-hot set, one slow client holding a pin across two compactions contests
/// essentially the whole page cache of a 47 GB box, and holds up to ~94 GB of invisible disk
/// through deleted-but-still-mapped files (`df` disagreeing with `du`). The cost of losing that
...
///
/// *An alarm is not a bound.* Without this, retention is publication rate × `pin_ttl_secs`, and at
/// the 300 s default with stage 2.2's per-flush `segments_version` advance that is tens of retained
/// bundles, each an `Arc<Bundle>` over mmapped segment files. Eight leaves an operator seven
/// depths of warning above [`DRAIN_DEPTH_ALARM`] before any pin is dropped, while capping the
/// retained set at something a 47 GB box can hold if the entries happen to be distinct prefixes.
///
/// A constant rather than a config key deliberately: stage 2.1's plan rule 2 lands every config
...
pub const DRAIN_DEPTH_MAX: usize = 8;
...
/// call and the fragment is cached, so rotating sessions is nearly free. The degradation past the
/// ceiling is deliberate and fail-*closed*: a forgotten session is charged for that pin again, so
/// it may hit [`EngineError::PinCapExceeded`] sooner than it should — never later. Total bookkeeping
/// is bounded at `DRAIN_DEPTH_MAX × MAX_HOLDERS_PER_ENTRY × 8` bytes ≈ 64 KiB.
const MAX_HOLDERS_PER_ENTRY: usize = 1024;

/// One superseded generation's geometry, kept resolvable until its TTL expires.
///
/// **Slimmed deliberately — NOT `Arc<Generation>`.** Holding a whole generation would also pin the
/// superseded *overlay* and *buffer*, which is exactly the state a pin must never use, and would
/// turn I11's "a pin never returns authorisation state" from a structural property of this type
/// into a rule policed by whoever writes [`PinManager::resolve`] next. The fields here are the
/// fields [`PinnedGeometry`] has, plus the three the drain list itself needs.
struct DrainEntry {
    prefix: String,
    ...
    ///
    /// **Identity, not a count, and the count is what the task brief sketched.** A `usize` per
    /// session cannot be right in either direction: a session re-presenting the *same* pin would
    /// consume cap on every request until it locked itself out, and reclaim would have no way to
    /// give the cap back. Both fall out for free once the holder set lives on the entry — reclaim
    /// drops the entry and its holders with it, and a re-presentation is a membership test.
    ///
...
/// What one reclaim pass removed — **Task 5's hook**.
///
/// `segments_version` is exactly the [`crate::cache::RowProjectionKey`] component Task 5's
...
/// prune would delete the very key it is about to ask for (plan, Task 5).
...
/// Operator-facing pin gauges — the drain-depth figure lifecycle §2.2 and the plan want on
/// `/control/status`.
///
/// Wiring these onto that endpoint needs `tessera-server/src/control.rs`, which stage 2.1's
/// allowlist gives to Track B; this track exposes them and reports the wiring rather than editing
/// another track's file.
///
/// Both fields are read from atomics and **this type is produced without taking the drain lock** —
...
pub struct PinStats {
    ...
    /// to this type must go through it, or that test silently stops covering rule 4.
    pub drain_locks: u64,
}

/// A geometry publication this engine refuses — see [`PinManager::check_publishable`].
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub(crate) struct PinManager {
    ...
    /// governed entirely by `pin_ttl_secs` and [`DRAIN_DEPTH_MAX`] — not by this. And a client can
    /// hold N drained geometries by rotating N sessions, since `Engine::authorise` mints a
    /// `token_id` per call against a cached fragment. So this is a per-session politeness limit
    /// that the spec requires and that keeps one *well-behaved* session's use of superseded
    ...
    /// Superseded geometries, oldest first, capped at [`DRAIN_DEPTH_MAX`]. Depth is expected to be
    /// 0 or 1 (see [`DRAIN_DEPTH_ALARM`]), so the linear scans below are scans of a one-element
    /// vector on the cold path — a `Vec` says that, where a map would imply a size this list must
    /// never reach, and the ordering is what makes "drop the oldest" expressible.
...
impl PinManager {
    ...
    /// never `overlay_version`. An overlay swap (any accepted suppression, deletion or predicate
    /// change) must not invalidate a pin; only a geometry swap does. `None` mints a pin naming the
    /// live generation's geometry.
    ///
    /// **The lock discipline is part of the contract, not an implementation detail.** Both arms
    /// below return without touching the drain mutex; only [`Self::resolve_drained`] takes it, and
    /// only when a presented pin does not name the live generation. Every admitted request calls
    /// this method, so a lock on the common path is a process-wide serialisation point on the
    /// viewport path — the exact class the concurrency workstream's F4 measurement exists to
    /// police. If a future change needs per-request state here, it needs a per-request-cheap way to
    /// keep it, not a mutex.
    ///
    ...
    /// Enforcing at mint would additionally have to take this lock on every request (breaking the
    /// discipline on [`Self::resolve`]) and would `422` an ordinary unpinned viewport, which is not
    /// a bound any caller could act on.
    ///
    /// **Two things this must never do, both of which look like helpfulness** (lifecycle §2.2, and
    /// I11's own wording). A restarted worker's drain list is empty, so every pre-restart pin
    ...
    ///   state is coldest.
    /// - **Never reinterpret the pin against current geometry.** This is I11's named failure
    ///   verbatim: "a row-space mask applied across a compaction boundary selects arbitrary rows —
    ///   not stale-restrictive but simply wrong". It would not fail closed. It would return a
    ///   viewport of unrelated items, silently, with a `200`.
    #[cold]
    fn resolve_drained(
        &self,
        presented: &PinId,
    ...
    ///
    /// **`segments_version` must strictly increase, and this is an authorisation-adjacent identity
    /// rather than a tuning bound, so it refuses rather than warns.** A publication that changes
    /// the *bundle* while leaving `(prefix, segments_version)` alone retires nothing, and every pin
    /// naming that identity then takes [`Self::resolve`]'s fast path and is answered against the
    /// **new** geometry — I11's named failure ("selects arbitrary rows, not stale-restrictive but
    /// simply wrong") reached through the API instead of through the resolve site the comments
    /// guard. Republishing an *older* `segments_version` (a rollback by pointer flip, design §10.2)
    /// is refused for the mirror reason: it would put a live geometry's identity on the drain list,
    /// where Task 5's `prune_generation` would later evict the live generation's own projections.
    ///
    /// `seg_id`s are never reused across compactions or prefixes (lifecycle §5.2, contracts §2.1),
    /// so strict monotonicity is what the format already promises; this only refuses to be the
    /// place it is broken.
    pub(crate) fn check_publishable(
        ...
        Err(GeometryRefused {
            ...
            reason: "segments_version must strictly increase; a bundle swap that leaves the pin \
    ...
    ///
    /// **An overlay-only swap retires nothing** — that is I11 stated as code: `overlay_version`
    /// moves on every accepted change batch, and retiring there would put one drain entry per
    /// suppression on a list whose depth alarms above 1, while pinning the same `Arc<Bundle>` the
    /// live generation is still using. [`Self::check_publishable`] already refuses the case where
    /// that could arise; this check is the belt to its braces.
    pub(crate) fn retire(
        &self,
        superseded: &Generation,
        next_prefix: &str,
        next_segments_version: u64,
    ) -> Vec<Reclaimed> {
        if superseded.prefix == next_prefix && superseded.segments_version == next_segments_version
        {
            return Vec::new();
        }
        let evicted: Vec<DrainEntry> = {
            let mut drain = self.lock_drain();
            if drain.iter().any(|entry| {
                entry.segments_version == superseded.segments_version
                    && entry.prefix == superseded.prefix
            }) {
                // Unreachable while `check_publishable` holds; keeping the earlier `retired_at` is
                // the conservative direction if it ever does not.
                return Vec::new();
            }
    ...
    /// to `PinManager` has to come through this function, and a rule-4 regression that locks on the
    /// common path shows up in the counter rather than in a benchmark six weeks later.
    ///
...
/// pre-suppression overlay.
///
/// Task 4 made that mistake *available* — [`PinManager::resolve`] can now hand back state from a
/// generation that is no longer live — and this type is what keeps it from compiling. The drained
/// arm builds one of these out of a [`DrainEntry`], which does not carry an overlay to give.
