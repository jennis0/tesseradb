...
impl Engine {
    ...
    /// Projection builds split by route, in [`crate::projection::ProjectionRoute::ALL`]'s order.
    /// Counts the background refresh's whole rebuilds as well as the request path's, so it can
    /// exceed [`Self::full_projection_builds`].
