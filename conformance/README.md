...
**734 pass, 5 skip, over 739 cases (2026-09-02, measured serially)** — the 32 that arrived on that
date are `test_suggest_differential.py`, the value-suggestion typeahead differential
(`docs/design/value-suggestion.md`, `conformance.md` §4.6's I2 and I12 rows). No failure this run;
the two `test_text_differential.py` timeouts below are recorded as load-dependent, and their not
reproducing here is consistent with that and not a claim about this machine's load at the time.
...
timeout** rather than a disagreement about entities: the box carried a load average around 30 from
concurrent work, and the same two cases fail the same way on `main` in a clean checkout, re-run
serially, at the same load. Recorded as measured rather than rounded to green — the number is
only worth having if it is the one that was observed.
...
| `tests/test_membership_growth.py` | decision 0127 (`artifacts-from-points.md` §6.1); I2, C8 | **A membership grown through the control plane.** One artifact is published with a third of the shape fixture's corpus under `tessera` addressing and grown in two more thirds by `PATCH /control/layers/{name}/artifacts`; the viewer plane's `masked_count` for each of three principals is then compared against the fixture's own term model, so a dropped slice or a count taken outside the mask disagrees. The receipt is a `tessera_id` and the count that joined, never an ordinal or a membership size; a re-sent slice joins nothing and moves nothing; an unknown key refuses the whole batch and a stale idset is a `409`, each with nothing applied. |
...
| `tests/test_suggest_differential.py` | I2, I12 (this surface's direction); C11 | **The value-suggestion differential** (`docs/design/value-suggestion.md`): `GET /v1/categories/{column}/suggest` against `suggest_fixture`'s Python fold oracle — its own dedicated corpus, built backwards from §4's fold rule (whitespace collapse, NFKC, a leading non-word run, an underscore-joined key with no title) rather than the mask catalogue, whose planted vocabulary values have no titles and cannot exercise the title or word-start entries. A prefix sweep (empty, one character, a whole key, a title word-start, a folded/NFKC variant, a miss) over both a `derived` and a `public` column, for a principal pair separated by exactly one hidden value (`omega`), asserts the served page equals the oracle's exactly — codes, keys, titles, §7's order and duplicate rule, and `match.field`/`start`/`len`. `counts=true` against the oracle's masked cardinality; `more` under both its causes (page-full, walk-budget-spent); a hidden and a hollow value simply absent, with a single-visible-member positive control (C11); `title: null` where none was authored; a viewport filter on the same session moving nothing suggested (I12's direction — the verb takes no filter operand at all); and the unresolvable-column refusal byte-identical to `/v1/categories`'. ⊘ The timing channel (C31) is declined here on the same argument `test_filter_differential.py` gives for C11's work-indistinguishability — see the module doc |
...
| `tests/test_multiview_differential.py` | I1, I2, I7 (order), I10/C17, I12 (scoped operand); `views.md` §1, §4, §5, §6, §9 | **The multi-view differential**, over `oracle/multiview.py`'s corpus: six views on one entity space of 6,144 items in six compartments — the plain view holding all of them, a group of three holding different subsets, and a **gated** group of two — each with its own frame and its own independently drawn layout. Masked tile counts per view against the oracle answering through that view's permutation, Morton order and frame; `served(view) == mask ∩ members(view)` as an **equality** on one token across every view, with the union over all of them equal to the mask; the same entity's `tessera_id` identical in every view and its position different in each; contracts §2.6's within-tile order in every view; and `views.md` §5's pinned leaf — bare under its own view, pinned by key and across views of one group, with per-view presence and the two refusals. A negative control fails if any two views serve the same per-tile counts. And **the gate** (§6), whose label is a real access label one principal holds and two do not: a failing principal finds the group on no surface — absent from `/v1/meta`'s roster whole, the viewer verb's status *and detail shape* those of a name nobody declared, both leaf spellings over its scoped attribute the unknown-column `422` that names neither group nor key — while a passing one is served exactly the ungated expectation, and the union-equals-mask equality holds per principal over the views that principal can reach. The gate's label is one compartment and the passing principal holds three, so an engine reading the gate as a row filter disagrees. ⊘ The gate's **work**-indistinguishability is a timing property and is not asserted here; a view's own label narrowing its group's is asserted at server level (`crates/tessera-server/tests/views_gate.rs`) |
| `tests/test_declared_equivalence.py` | build = ingest into an empty database (CLAUDE.md) | **Anything declared at a running service serves what a build that declared it serves.** Each case builds one deployment whose declaration carries the thing and one whose declaration omits it, then declares it through the control plane, writes its data (`/control/values` for items the build holds, `/control/ingest` for the rest, publication and growth for layers) and waits for the flush. Every principal's answers to one battery are compared, after the live declaration, a restart, a fold and a restart after the fold: `/v1/meta`, category lists and the typeahead, the viewport at several zooms with and without a filter and a highlight, items, browse and artifact cards. The cases are entity attributes of every declarable type, live vocabularies, a plain view, a view group with scoped families and a view of it dropped and recreated, and layers with a hierarchy and a fractional membership requirement. Items are compared by source id and artifacts by key (`declared_fixture.py` rekeys in one place). Each engine difference found is a strict expected failure, whole where the live declaration cannot be made and narrowed to the answers it touches where it can |
