...
- **Refusals** — an unknown *column* is a `422`; an unknown *value* is not; `none_of` and
  `match` are `422` because they are unbuilt (decision 0059), and an operator outside the
...
def test_the_unbuilt_operators_refuse_by_name(catalogue_server, sweep_cases):
    """Decision 0059 and decision 0013: `none_of` and `match` are specified and unbuilt, so
