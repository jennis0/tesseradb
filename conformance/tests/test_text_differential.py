...
@pytest.fixture(scope="module")
def column(catalogue_bundle) -> txt.TextColumn:
    """The whole corpus's prose, tokenised once through the shipped analyser.

    **Keyed by entity id, generated by source id.** `abstract_of` takes the source id the fixture
    ...
    prose = {
        ...
        if (value := cat.abstract_of(source)) is not None
    }
...
def test_the_text_column_has_the_shapes_its_catalogue_entries_need(
    catalogue_bundle, column, tokens_of
):
...
def test_a_suppressed_entity_is_not_served_though_its_terms_are_in_the_postings(
    ...
    # the verify.
    # No `abstract_of` guard beside the phrase test: the column holds an entry only where the
    # fixture planted prose, so every key here has some. It used to read `abstract_of(e)`, which
    # takes a **source** id and would now be asking about a different item.
    victim = next(e for e in sorted(column.tokens) if column.has_phrase(e, pair))
