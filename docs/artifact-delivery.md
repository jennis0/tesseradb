...
| Where supplied content **lives** — the storage half of the above | open | **Stage 3** | separable from the edit route and still needed to store label text at all. Editing turns out not to constrain it: a record lives in one 256 KiB block, and an edit is a republish of the artifacts' own extent, never a second layer (which would serve the pre-edit text silently) |
