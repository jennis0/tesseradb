...
A membership now **grows** as well as being published whole, which is what a point joining a cluster
at ingest needs and what a build reading a member table has always done
([decision 0091](decisions/0091-build-is-ingest-into-an-empty-database.md)); ⊘ what a point cannot
yet do is *say* which artifacts it belongs to on the wire. What does not exist yet beyond that is
membership by predicate, runtime artifacts, and the filter, search and scale work — Stages 6 to 8
below.
