...
refinement — **every grid in the scale memo's §7 superseded pending the corrected probe**, 0093
amended with the owner's row-major exception, and two leak-register annotations approved
(architecture **r49**).
...
constraint that ordered the wave. The serving path itself stays
