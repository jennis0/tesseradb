...
| **Artifacts from points** | **stages 1 and 2 of 3 done** 2026-08-19; **the third is blocked on a ruling** 2026-08-20 (`artifacts/stage-4`) — [the design](design/artifacts-from-points.md) | a clusterer's own output builds a layer: one integer per point or one list per point, noise included, and no artifact table required | **§2, §3 and §4 built and gate-green.** The membership route needed no new surface and is asserted so — a `[layer.members]` block over the points file builds the same bundle, byte for byte, as the same layer declared with a member table. Added in the first stage: an integer key column canonicalised to its decimal spelling (converted once per artifact, never per point); a null key and exactly `-1` skipped, counted and reported rather than refusing the build; and `value_set = "open" \| "closed"` on `[[layer]]`, closed being today's roster rule, open making a key no artifact declares create one. **In the second, a list key column**, whose entries are the artifacts the point belongs to and whose positions mean what the layer's `hierarchy.kind` already declares — one per level under `stacked` and `tiered`, a lineage under `nested`. The same byte-for-byte assertion holds both ways: a lineage column against an artifact table with a `parent` column, and a fixed-length column against a member table with a `level`. A child named under two different parents refuses the build; a shape disagreeing with the declared kind refuses rather than guessing; a null or `-1` entry places the point at no artifact at that level and links nothing across itself. ⊘ **§6 (the ingest half) is the remaining stage and it is blocked, not merely outstanding** (2026-08-20; [§6.1](design/artifacts-from-points.md) and r4 carry the investigation). Two obstacles, neither of them wiring. **The corpus contradicts itself**: [`annotation-write-cycle.md`](design/annotation-write-cycle.md) §3.4 rules that an ingested point enters an enumerated membership *never*, and §5 of this design rules that it joins — both normative for artifact semantics, so one is wrong and which is a ruling. **And nothing can grow an existing artifact's membership**: publication is append-only and refuses a key the level holds, the fold's two passes only remove, and the one durable record carries the set entire. The piece that decides the shape is the packing bookkeeping rather than the record — a grown record below `published_through` is never repacked and the rotation pin releases at the next flush, so the acked join is lost on restart with nothing saying so. The fold is the candidate home, because the row form covers base rows and the join has no observable effect before a fold whatever route carries it. **Nothing was built**: a column accepted at `/control/ingest` whose effect is unbuilt either drops the caller's membership silently or answers 422 for every batch, which an undeclared column already does |
