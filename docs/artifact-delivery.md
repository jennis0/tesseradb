...
| **5** Trees, levels and the cut | in progress (2026-08-17, `artifacts/stage-4`) | a passing child sits beneath a failing parent under the proportional criterion and never under the absolute one, and two budgets agree on every artifact both return | **the mechanism is built and both halves of the check are met in the engine.** A hierarchy's edges are inline on the artifact record — the parent direction durable, the child direction built per level at serve time, so no deletion has to keep two copies of one fact agreeing. The build reports every edge whose child escapes its parent and refuses the ones that describe no tree; stray members in the parent are the normal case and not a violation, at the measured 20–25%. The cut runs after the verdicts and can only serve fewer: where a parent and a child both pass the child is drawn, and a budget is met by climbing to a **passing** ancestor rather than to a depth — the defect an integration test caught, where a suppressed root blanked its children's regions entirely. The generator has its **edge arm**, closed form in both directions and non-covering by construction. Open: the check on the **real condensed tree**, which is what makes this more than plumbing |
