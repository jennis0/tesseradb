...
close, which was 0091's last obligation. One tail is
owed on the cut and is ruled: a dependent artifact is dropped when the cut drops what it depends on,
server-side, with the attachment never reaching the wire (Stage 5 below). What does not exist yet
beyond that is membership by predicate, runtime artifacts, and the filter, search and scale work —
Stages 6 to 8 below.
...
**One tail is owed, ruled by the owner 2026-08-20 and not yet built: a dependent is dropped when
the cut drops what it depends on.** [Decision 0089](decisions/0089-a-dependency-edge-carries-deletion-and-visibility.md)
...
The machinery is in place: `serve_artifacts` already holds the whole response before resolving
parents, in a `served_at` map keyed by `(layer, level, ordinal)` — which is exactly the triple an
attachment carries. **The trap is the request that names the dependent layer alone**, where a naive
lookup finds no target and drops every label; refusing a legitimate call is outside the disclosure
surface, so the condition is that the target's layer is *in this request* and its target is not in
the response. One response never contradicts itself; a request for labels alone is unchanged.
