...
**A third route removes the artifact count from the request path entirely, and it needs no new
storage.** A single-valued attribute predicate *partitions* the corpus — the column's distinct
values are its artifacts and every point carries one — so the column the predicate names is already
the row→artifact map. One pass over the visible rows, incrementing a counter per row, answers every
artifact at once: **35–37 ms over the whole map at every layer size from 64 artifacts to a million**,
measured and asserted against the per-artifact loop's own answers. It is two passes with different
domains, and that is a disclosure rule rather than an optimisation: a masked count is over the whole
membership (`annotations.md` §4.2) so its pass walks the mask, while candidacy is against the
viewport.

So this is a **route choice with a crossover near 3 000 artifacts**, exactly the shape the filter
surface already has — below it the per-artifact loop wins (10.9 ms against 35.8 ms at the demo
corpus's 263), above it the histogram is not close. It is a latency choice with no disclosure
content: both compute the same quantities from inside `M_auth`. ⊘ Two further reductions are
modelled rather than measured — the counting pass is a function of `M_auth` and the layer and not of
the request, so it can be held per session on the mask fragment's own cadence; and with counts
coming from the column, row forms are needed only for the artifacts actually **served**, which the
budget bounds, so the generation-move cost becomes a handful of lazy projections. ⊘ A multi-valued
column does not partition, and a spatial predicate has no column — but its membership is Morton
ranges, so its count is a `range_cardinality` and cheap per artifact anyway.
