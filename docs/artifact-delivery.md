...
| **6** Predicate membership and the serving layouts | not started — **its cost discussion is held and measured**, 2026-08-20; **its rulings taken and its review dispositioned**, 2026-08-21 | one layer built by rule and by list returns identical masked counts for every principal and every viewport, and a layer with no row-space locality is served by a layout that does not walk its artifacts | **the row form is cached at a generation move**, as an enumerated layer's is ([the probe](../crates/tessera-bench/src/bin/predicate_membership_cost.rs)): deriving per request by crossing costs **7 900×** a cached count at the demo corpus's 263 artifacts — 6.9 s against 0.9 ms — because a crossing is `rows × artifacts` where a count is `containers × artifacts`, and the caching arm's whole move cost is repaid by one request. Filtering before evaluation stays refused. **A second route removes the artifact count from the request path**: a single-valued attribute predicate partitions the corpus, so one masked histogram over the column it names answers every artifact at once, flat in the layer size — **175 ms against 462 ms** at 10⁶ artifacts over 10⁷ points. The column is `attrs/`'s entity-space `ValueColumn`, not the render table. A crossover near 30 000 artifacts at that corpus size, no new storage, and nothing on the wire naming which route served. **The scale campaign then found that route is the general one** ([the memo](design/artifact-serving-at-scale.md), [the probe](../probes/2026-08-20-artifact-serving-scale/README.md)): what decides cost is row-space **locality** rather than membership source, so a **row-major** layout — a label per row where the layer partitions, a list per row where it overlaps — serves every scattered shape, and at 10⁹ points it is the only layout that fits at all, 4 GB against 78.5. **Three rulings, 2026-08-21**: no declared bound, and every build reports blocks per artifact ([0092](decisions/0092-the-build-reports-a-layers-shape-and-no-layer-carries-a-declared-bound.md)); nothing per token over the artifact population, containment coming from a build-time partition over terms ([0093](decisions/0093-nothing-is-materialised-per-token-over-the-artifact-population.md)); the layout chosen automatically per (layer, level), pinnable per layer, re-evaluated at each fold ([0094](decisions/0094-the-serving-layout-is-chosen-at-build-and-re-evaluated-at-the-fold.md)). **The per-request bound is withdrawn, not deferred.** **The adversarial review ran and is dispositioned** ([the record](evidence/memos/2026-08-21-artifact-serving-scale-review.md)): two fail-opens found in the unbuilt design and amended — the settled half of the candidate walk tested containment where it needed the mask, and the containment partition's deny correction is its acceptance test rather than a refinement — 0093 amended with the owner's row-major histogram exception, and two leak-register annotations approved (architecture **r49**). **The re-measurement has run** (2026-08-22, medians of three runs of three iterations on the corrected probe): the masked candidacy the old routes omitted is real and is now the dominant term of every figure — the whole-map cell at 10⁷ artifacts over 10⁸ points was 31.7 ms and is **553 ms**, of which candidacy is 549 — and what survives is the conclusion, the design running **10–25× ahead of the per-artifact loop** with its worst measured cell at **905 ms** on one core. **The cost inversion is fixed**: the design's dear cells are broad masks and its narrow principals run sub-millisecond, where the per-artifact loop's worst cell at 10⁶ artifacts is a 3.1% mask paying 4.4 s. **And the campaign's largest design finding is the expression census**: 32 distinct containment expressions under `annotations.md` §8.1's per-term authoring, against **one per artifact** for generating sets drawn across the demo corpus's real 54,794-signature distribution — so the partition's wide-viewport union route exists **only** for per-term-authored sets, and for drawn sets containment degrades to a viewport-bounded per-candidate evaluation whose wide zooms the build wave has to price. **The serving build is under way on `artifacts/serving-build`, two stages landed** (2026-08-22): the row form split at its cost seam (`ArtifactRecords` always built, `MembershipRows` route-dependent), and the **containment partition built** per (artifact, rank) — expressions interned at `u16` with checked promotion, the two faces behind one type, the deny correction live on the accept path, the partition gated to the builtin plugin and absent under any other, its durable form fold-written at a **per-level version coordinate the manifest now carries** (a restart adopts at exact equality and recomposes otherwise, proved in both directions; the fold's retirement seam closed by omitting moved levels from both lists) — then the **tile index and per-artifact extents integrated into serving**: the per-ordinal sweep replaced by the walk, every candidate paying a masked probe, holes and empty projections distinguished by sentinel, the `everywhere` set first-class, one snapshot per level, extents-only durable (`TSTI`, the node hierarchy derived in the validating pass), and a differential test pinning the walk to the sweep ordinal-for-ordinal across shapes, principals, overlays and a mid-sequence growth. Gates read **1,878** then **1,898 passed / 0 failed / 11 ignored**. **Stage three followed the same day**: the `ServingLayout` enum recorded per (layer, level) and re-evaluated at every fold, the `layout =` override refused where inapplicable and pinned across folds (a flip drops the old form's projections, straddled correct by test — including one driven by the heuristic itself at 700,000 rows), the `TSLB`/`TSLL` row-major columns fold-written and mapped with the derived declared-size column answering the proportional denominator on those levels, and the masked-count histogram as its own byte-budgeted cache whose deny edge is the overlay version — a suppression rotates the key, an unsuppress re-derives, and `delete → suppress → unsuppress` stays deleted through it. The twin differential pins the two layouts identical artifact-for-artifact across principals, viewports, denies, growths and a fold. Gate **1,928 / 0 / 11**. ⊘ The artifact-major row form is still built beside a row-major column — the residency half of §5.1 waits on containment's projection-loss and the computed geometry reaching memberships another way — and the layout thresholds are provisional pending the bracket re-run. **Stage four closed the chain and this row's own check** (2026-08-22): predicate membership serves — an attribute layer's artifacts minted from its column's distinct values (build and ingest, one key rule, publication into it still refused; a suppressed value's key never re-mints), served row-major from the value column split at the cadence seam (base keyed on the level version, live tail on the geometry) so **a point ingested with value v counts at its flush with nothing rebuilt**; a spatial layer's boxes decompose to Morton ranges at a declared depth (`[layer.shape]`, depth 1–16, the ranges being the membership per decision R3 recorded in the review) counted by `count_range`. **The check is met**: the same layer by rule and by list serves identical counts and sets for five principals over six viewports through the real `tessera build`, pinned to the generator's own oracle. Gate **1,956 / 0 / 11**. ⊘ Computed content on predicate layers stays refused; the proportional denominator stays open | ⊘ The proportional criterion's denominator stays open (its row-major answer is stage three's declared-size column); the two cache-cadence defects the build depended on are **fixed and merged** (2026-08-21) — one write invalidates one level in one store rather than every view's every layer, and the lineage is held per generation and warmed at the fold |
| **7** Scale and the write cycle under load | not started — **pulled forward ahead of runtime artifacts** (owner, 2026-08-21) | 10⁹ points carrying 10⁶ artifacts, with 10⁷ as targeted probes, serve inside budget for many concurrent principals, fold under load, and leave the write path correct at every interleaving | **the serving half is measured on the corrected probe** (2026-08-22; [the memo](design/artifact-serving-at-scale.md) §7, [the probe](../probes/2026-08-20-artifact-serving-scale/README.md)), medians of three runs of three iterations. The target's two axes are measured one at a time: **10⁷ artifacts over 10⁸ points runs 8.4–905 ms** across the whole grid of principals by zooms, and **10⁶ artifacts over 10⁹ points runs 0.75–209 ms**, against 11–26 s and 1.8–8.3 s for the per-artifact loop at the same cells. Both hold the one-core one-second budget, and flatness in corpus size holds at 10⁶ artifacts (132 → 131 ms full-map from 10⁸ to 10⁹ points, worst cell ×1.5 for ×10 corpus). ⊘ **The corner where both axes are large is a negative result on this box**: 10⁹ points with 10⁷ artifacts reached 45 GB resident with all 12 GB of swap exhausted and no phase progress for three hours, and was killed — its cost there is **modelled from the flatness at 10⁶ (~550–900 ms) and needs a larger machine than this 47 GB one to measure**. Only the **cut** is in the engine: 1 008 ms → 188 ms by flattening the per-node lineages, which is where the peak RSS of 1 078 → 470 MB belongs, and then → **3.05 ms** at 10⁷ by the downward walk; serving exactly what it served before, checked against the reference implementation over random trees. What this stage owes is everything else — the generator's artifact arm reaching disk, the campaign matrix with its 1/8/32/128 concurrency sweep, **serving during a fold** at 10⁹ (the named gap; the fold's artifact pass is measured unloaded only), the write-path interleaving battery, and the configuration-matrix, census and conformance fill. **The campaign ran against the built engine** (2026-08-22/23, [the memo](evidence/memos/2026-08-22-artifact-scale-campaign.md), [the record](../probes/2026-08-22-artifact-serving-e2e/)): **98 of 98 census cells exact** across three corpus sizes, both directions, over HTTP; the design's grids reproduce end to end **after the first fold** (worst cells 2,430 → 221 ms and 1,464 → 139 ms when the fold's re-evaluation flips the layouts the build did not choose); the twin and its predicate spelling are the same number; spatial is 1 ms flat; **the fold under 32 live sessions runs at 0.98× its unloaded duration** with nothing shed, though a live session's p99 spikes to ~59 s during it; ingest costs the read path nothing at 47 k rows/s. The envelope: throughput peaks at 8 sessions, cores saturate at 32, and **memory binds** (~76 MB per session at 10⁷ points, 221 MB at 5×10⁷). **Five engine findings reported, none fixed in the measuring**: the build records a layout its own report contradicts (0094's build half is missing — the fold alone re-evaluates); a cold request is truncated at the stream deadline while the row form builds, silently at the server; a 5×10⁷-member membership fails its own Roaring round-trip on a path documented unreachable; `tessera build`'s peak RSS ignores `--memory-budget` (OOM at 2.5×10⁸); and a wide response over a large flat layer is unbounded (254.6 MB at `artifact_budget = 100`). **The fix wave answered the findings** (2026-08-23, `artifacts/fix-wave`, merged): the layout is now chosen **at the build as well as the fold** — a post-bundle artifact pass observes each level against the bundle's own row space, reports blocks per artifact and the everywhere fraction (0092's figure, discharged at build time at last), records the pick and writes the fold-grade derived files through one implementation shared with the fold — so **a fresh bundle opens adopting everything and composing nothing**, which is what closes the silent stream-deadline truncation (a shed is also logged now, never silent). The trigger itself moved to the **everywhere fraction** (owner, 2026-08-23) — the campaign showed cost tracks extent spread, not container count — at a ⊘ provisional 0.25, unsettleable by the one-sided bench arm; a two-sided fixture is the recorded remaining measurement. The membership refusal at 5×10⁷ was **corrupt before it was encoded** — an intermittent array-container corruption below this stack (CRoaring 4.7.1 or this machine's memory; the campaign's parquet panic is a second such symptom and **the box wants a memory test**) — worked around soundly by sort-then-append (6.4× faster, removes the observed path), the round-trip check kept, and the recurrence tests naming which half moved. The build's other residency peak — the entity-order tail, dominated by 24 B of `String` header per variable-width value — has a **footer-derived lower-bound model that refuses over budget before the first pass** and warns in the band below; the unbatchable stages and their scaling are stated. The wide flat response (254.6 MB) is **documented and deferred** (owner, 2026-08-23), streamed continuation the likely eventual shape, owned by the client epic. **The bracket verdict overturns the threshold's premise**: blocks = 8 was not an anomaly — artifact-major *improves* through 8/10/12 as the `everywhere` set grows, so no crossover exists in the 2–12 band and `ROW_MAJOR_BLOCKS_PER_ARTIFACT = 10` flips away from an improving layout. **Acceptance, clause by clause**: 10⁹ points **not reached** (four walls — the build's RSS, the membership encoding failure, and disk); 10⁷ artifacts probed on the artifact axis; serve-inside-budget met at 10⁵ artifacts and not above; fold under load **met**; write path **met** for what the campaign tests; census exactness **met**. [The plan](evidence/memos/2026-08-21-artifact-scale-plan.md) |
...
| **10** Shape membership — box, circle, ellipse and polygon, one semantics | **all four stages built** 2026-08-29 (`artifacts/shape-core` → `artifacts/shape-type` → `artifacts/shape-wire` → `artifacts/shape-region`, not merged to main) | a published boundary set and a drawn lasso are one mechanism asked at two times (`polygon-membership.md` R13), every kind exact, a point ingested inside a shape a member on the next request | the design is [`design/polygon-membership.md`](design/polygon-membership.md) r5, eight owner rulings taken and the review dispositioned 2026-08-29. **Stage 1, the core**: `tessera_spatial::shape` — the four kinds, the descent, the tie rule, WKB/WKT, the canonical grid form, the bytes. **Stage 2, the artifact type**: `shape.kind` with `depth` deleted and refused naming §6.1; `default_space`/`space` (`view` only, `wgs84` refused naming `projections.md`); the per-view blob (`ArtifactShapes`, tag 3); `tessera_engine::shapes` holding each level's shapes, depth-8 index and per-segment resolved pieces — built at open and at every publication into the layer, filled by the flush on the pool before its publication, re-resolved at the fold and the merge, never on a request (a request that finds a piece missing resolves it and warns); the resolved rows take the enumerated layer's layouts, so a spatial level is picked and pinned like any other; `tessera check` sizes the decomposition from the geometry alone; `PUT /control/layers/{name}/artifacts` takes the same row fields and returns the same report; `disclosure.json` spells `spatial:<kind>`. **The oracle**: `conformance/tests/test_shape_membership.py`, a plain even-odd walk on the quantised grid with on-edge inside, agreeing with every masked count and every `membership:<layer>` value under three principals at the build, after a flush that adds points on edges and vertices, and after a fold. **Measured on Overture part 0** (4.6×10⁶ places, 17,551 divisions, 1.25×10⁷ vertices): `check` 14.9 s; build 87.6 s wall and 1.87 GB peak RSS against the enumerated declaration's 55 s / 1.33 GB, of which the resolution is 11.8 s (4.3×10⁶ rows tested one by one in boundary cells, 1.4×10⁷ interior admissions) and the layout pass 12.6 s; held 57.7 MB (2.5×10⁶ interior tiles, 3.4×10⁶ boundary cells) beside 150 MB of canonical bytes; the zoom-0 three-country request naming the layer serves 8,451 divisions (8,448 from the lineage join) in 182 ms cold and 21–24 ms warm; the fold re-resolves the segment in 2.6 s inside a 4.5 s fold. **Persisted 2026-08-29** (owner ruling): the build and every fold write what they resolve — the row-major column, or a `shape-rows` row form keyed by segment id, row count and level version — and every artifact's decomposition in a `shape-held` file keyed by level version and the canonical bytes' digest; the open claims both and resolves only flushed segments, refusing (never adapting) a file whose key differs. Process start to `/readyz` on the same part: **11.3 s → 0.69 s**, and 12.9 s → 0.71 s after a fold and a restart. The build's report states both artifact counts — 17,551 with a shape, 17,544 with a row — and names the seven that differ (two mainland-only countries, two generalised polygons, three metres-wide polygons whose one place is inside at source coordinates and outside at its stored `f32` position). Gate: 2,127 Rust tests, clippy, layers, clients, python, doc-links green; `conformance/tests` 433 of 433; `conformance/suite` 183 passed, 4 skipped, and the seven `test_total_verification` cases error at fixture setup because the materialised corpus lacks the `flat_artifacts.parquet` its own declaration names — a harness defect that predates this work. **Stage 3, the wire and the client** (`artifacts/shape-wire`, 2026-08-29, on the owner's ruling that an artifact has **one drawn geometry** of a declared kind — derived, predicate or authored — `polygon-membership.md` §7.1): `hull_x`/`hull_y` become `shape_x`/`shape_y`, `list<list<list<uint32>>>` (parts, rings, vertices), omitted from the schema when no served layer draws or the request narrowed the shape away; the ask vocabulary is `centroid, box, shape` (`hull` stays the declaration's word, `ComputedProperty::parse_ask`); `/v1/meta` publishes `shape: derived | predicate | authored | null` per layer (`LayerDeclaration::drawn_shape`) and `disclosure.json` records it; a predicate shape is the held canonical shape filtered per request to the vertices above the cell a pixel covers at the request's depth under the 2,048 guard (`tessera_engine::shapes::served_rings`, `Shape::rings_guarded`), the guard's firings counted into the trailer's `stage_ns` companion as its 23rd field; the three authored content kinds (§6.1) are read at the build and at `PUT /control/layers/{name}/artifacts` through the membership shape's own reader and cap, held as the canonical per-view bytes in the content slot (`ArtifactShapes::content_text`), served through the same columns and blanked in `content`; a layer declaring two drawn geometries is refused at parse (`DeclarationError::TwoDrawnGeometries`); `POST /v1/artifacts/{id}` carries `shape` under the same nesting and takes an optional `zoom`. The client (`@tesseradb/client`, `deck`, `components`) decodes three levels and refuses the old names, holds the kind from the meta, keeps a predicate or authored shape across a change of principal and never a derived one, draws every kind through one polygon-with-holes path (only the derived kind smoothed), picks by even-odd over each part, and shows the kind on the card with *filter to this* greyed for stage 4. Contracts §3.2 r45, `artifact-shapes.md` r8, the OpenAPI description, `annotations.md` §8.6 amended. **Evidence** in [`evidence/screenshots/2026-08-29-shape-wire/`](evidence/screenshots/2026-08-29-shape-wire/README.md): the Overture one-part boundary layer at the country and city scale under two principals — the same division's shape byte-identical under both, generalised to the depth asked at — and the taxonomy layer's derived hull differing per principal, from `clients/ts/viewer/smoke-shapes.mjs` headless and headed. ⊘ left: the held edge table (§6.3), the sizing script, the memo's superseded-by note, the conformance matrix row, the client's golden captures (r44 columns, refused by name until recaptured), and `reference/oracle/wire.py`'s `hull_x` reading — outside the track's allowlist, reported. Stage 4 (the region leaf) is the remainder **Stage 4, the region leaf** (`artifacts/shape-region`, 2026-08-29; `selection-operand.md` promoted to Normative r2): `FilterExpr::Region` — `polygon`, `bbox`, `circle`, `ellipse` with `space`, or `artifact` — parsed and canonicalised at the wire against the view's extent (`filter_dto::parse_region`), routed row space over the whole view (`RowExpr::Region`, `RowExpr::NotInRegion`; `FilterRows::Complete` for a tree of regions and projected verdicts, `Viewport` when a render leaf or a per-tile crossing bounds it); `tessera_engine::region` holds the decomposition per `(view, generation, canonical digest, max_region_cells)` in a second `SingleFlightCache` shared across principals and pruned at every geometry swap, and tests each boundary cell's rows under the request's composed mask — `rows_under(mask, …)` takes the mask so the path cannot run without one, and the probe's `filter_matched` is not counted for a tree carrying a region; the leaf by artifact through `Engine::gated_artifact`, the predicate `/v1/artifacts/{id}` now shares, an empty operand for an unknown, suppressed, unreachable, withheld or authored-shape artifact; `x-tessera-region: exact | cover; depth=<d>` from a head delivered after the filter and before the sweep; `region` reserved at the build; `max_region_vertices` (10,000) and `max_region_cells` (262,144) on `/v1/meta` and in `[serve]` with `region_cache_bytes`. The client: `region.ts` reduced to the leaf's spelling, the header's parse and the server's quantised even-odd predicate; a selection composes into every request's filters and its count is the frame's `matched` sum, exact when the verdict is and the replica holds the shape's extent; *filter to this* and *outside this* live on the card, *outside* on the selection panel. Second readers: `oracle.filters.RegionColumn` and `conformance/tests/test_region_leaf.py` (seven tests), `crates/tessera-engine/tests/region_leaf.rs` (four). Evidence in [`evidence/screenshots/2026-08-29-shape-region/`](evidence/screenshots/2026-08-29-shape-region/README.md). ⊘ `architecture.md` §8.2's marker on the operand is the owner's to remove; `POST /v1/region` and the runtime-artifact path stay unbuilt. |
...
[the ladder's arXiv rung](../test_corpora/arxiv/README.md), which publishes all three shapes over
the same points —
