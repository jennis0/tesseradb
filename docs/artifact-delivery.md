...
| **Artifacts from points** | **stage 1 of 3 done** 2026-08-19 (`artifacts/stage-4`) — [the design](design/artifacts-from-points.md) | a clusterer's own output builds a layer: one integer per point, noise included, and no artifact table required | **§2 and §3 built and gate-green.** The membership route needed no new surface and is asserted so — a `[layer.members]` block over the points file builds the same bundle, byte for byte, as the same layer declared with a member table. Added: an integer key column canonicalised to its decimal spelling (converted once per artifact, never per point); a null key and exactly `-1` skipped, counted and reported rather than refusing the build; and `value_set = "open" \| "closed"` on `[[layer]]`, closed being today's roster rule, open making a key no artifact declares create one. ⊘ **§4 (lineage from a list column) and §6 (the ingest half) are the remaining two stages** — `value_set` rides the layer declaration so the write path has it to read |
