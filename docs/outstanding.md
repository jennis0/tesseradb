...
- An authorise that arrives while another is building the same fragment is refused (`FragmentCacheError::Building`, a 500 today). The engine's other single-flight caches wait within a budget instead, and `tessera-cache` now gives the fragment cache the same choice. Its `waits_satisfied`, `waiters_now` and `prune_scanned` counters stay at zero for the fragment cache because it never waits or prunes. Decide whether authorise should wait.
