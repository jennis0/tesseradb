...
- A flush of a view that borrows a group's scoped columns through a sharing door writes its scoped extents under the owning view's name and incarnation (`FlushContext::scoped_view`, `scoped_incarnation`), and stamps them in the manifest with the flushing view's own incarnation. The same stamp goes on the scoped text extent and on the `ScopedColumn` entry. The two incarnations are equal until one of the two keys is dropped and created again; after that a reopen compares the stamp with the owning view's incarnation, finds a mismatch and skips the extent, so the running process serves values a restart of the same bundle does not. From reading; no test builds a sharing door with a recreated key. Decide whether the stamp becomes `scoped_incarnation` in all three places.
