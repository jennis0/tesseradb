...
- `compose` walks the whole ingest buffer on every request and keeps a result only for a buffered entity that already has a published row, which is rare. A viewport is 0.015 ms with an empty buffer and 3.1 ms with 40,000 rows buffered. The generation could carry, per view, the buffered entities that have a row, so the walk stops depending on the buffer's size.
