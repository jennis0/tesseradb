...
**What each gets:** the branch diff, the plan's task text, the SDD brief and the worker's report, and explicit paths into `.ignore/` (default tooling skips it). Each is told it has no stake in the work being right, must cite `file:line` rather than assert from memory, must rank CRITICAL / IMPORTANT / MINOR with a concrete failure scenario per finding, and **must say where the work is right when a reviewer would expect otherwise** — a review that only lists problems is not calibrated and cannot be triaged.
