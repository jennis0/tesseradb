...
**Terminology note — this key deliberately avoids the word "epoch".** The corpus already carries two distinct epochs: the **identity epoch** (contracts §2.2 — advanced on identity-breaking events; the one spec §8 discusses) and the **deny epoch ledger** (lifecycle §3 — the write-path structure retiring deletion denies). An earlier draft of this spec named the table key's third component "epoch", introducing a third meaning; it is `flush` throughout, and "pending table" is the noun for `(slice, residual, flush_i)`.
...
| Contracts §2.2 | Identity-epoch-advance rule relaxes to "reallocation or rotation" (with Tier 1's own proposal); identity-preservation attestation |
