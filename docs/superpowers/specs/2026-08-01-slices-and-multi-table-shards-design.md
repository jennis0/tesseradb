...
**Terminology note — this key deliberately avoids the word "epoch", and the corpus's own duplication is proposed for repair.** The corpus carries two distinct epochs: the **identity epoch** (contracts §2.2 — advanced on identity-breaking events; the one spec §8 discusses) and the **deny "epoch ledger"** (lifecycle §3 — the write-path structure retiring deletion denies). An earlier draft of this spec named the table key's third component "epoch" too, introducing a third meaning; it is `flush` throughout, and "pending table" is the noun for `(slice, residual, flush_i)`. The residual two-way ambiguity is itself ruled worth fixing (owner, 2026-08-01): **lifecycle §3's "epoch ledger" is renamed "retirement ledger"** — it is purely internal, so the rename costs a document edit and mechanical code-identifier changes, while "identity epoch" is wire-facing (`identity_epoch` in `/meta`, the ingest `epoch` field) and keeps the word. Spec §11 carries the amendment.
...
| Contracts §2.2 | Identity-epoch-advance rule relaxes to "reallocation or rotation" (with Tier 1's own proposal); identity-preservation attestation; **rotation split into roll (multi-epoch key retention, epoch-tagged identifiers translate, no break) and revoke (today's 409 semantics)** |
