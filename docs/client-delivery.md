...
| S1 | `@tesseradb/core` and `@tesseradb/components`: `TesseraClient.suggest` over `GET /v1/categories/{column}/suggest` (`value-suggestion.md` §5.1, contracts §3.2 r72); the store's `suggest` action, debounced at 120 ms per column, idempotent per `(column, q)`, discarding a response whose echoed `q` is stale, and retrying a `429` floored at 250 ms and capped at five; the category filter control as a typeahead — the server's `match` span drawn as `<mark>` over the served string, never a client fold; chosen values as chips; `more` as *type more to narrow*; a keyboard-operable listbox. The store's enumeration path for category values is deleted (the legend resolves codes through `resolveCategoryCodes`). Counts are never requested | `check-clients.sh` green (core 370, components 113); store tests for churn, a cleared refusal and the bounded retry | the server verb (campaign track `server`) for a live smoke — ⊘ not yet smoked live | **built** 2026-09-02 (`vs/client`, refereed, four fixes). No design board exists for this control: it reuses the panel's tokens, parts and icons, and no screenshot comparison was made |
