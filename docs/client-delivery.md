...
| S12 | a served hull is **one ring**, so a membership that is two separated clouds is drawn as one polygon over the gap between them | the contours the map draws; the pick path's one-polygon-per-artifact assumption | **built 2026-08-28** (track `rings`, branch `design/rings`; owner ruling 2026-08-27). The engine groups the visible members at α and digs a ring per group; `hull_x`/`hull_y` become `list<list<uint32>>`, which is a **breaking** wire change made so a single-ring reader fails its downcast rather than concatenating the rings. `artifact-shapes.md` is promoted to normative with all six rulings folded in — the family stays the dig, because a Rust Delaunay costs 1.4–1.5 s on the 2.4M-member artifact against 0.16 s for the whole dig, and there are **no holes**. On `clusters/hdbscan`: 215 rings over 197 artifacts, 4 carrying more than one, +1.7% hull bytes, +8% time, and the tightest shape 0.290 → 0.093 of its wrap. **The client is not changed** — what it must do is `artifact-shapes.md` §9: one outline datum per ring all carrying the artifact's `tessera_id`, a row→artifact map in place of the pick path's index match, `extentOf` untouched, and a decode that descends two levels and checks the two axes agree. One file outside the track's allowlist is required and was not touched: `docs/openapi/tessera.yaml`'s `ArtifactResponse.hull` |
