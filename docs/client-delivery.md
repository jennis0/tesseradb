...
| cache 2 | the artifact channel **holds its payloads**: the served identifier list is replaced wholesale as it always was, and the payloads accumulate beside it, keyed `(layer, tessera_id)` and dropped whole when the identity key or the content key they were fetched under rotates (obligations rule 7). An ordinal is taken once when a payload enters and released when it leaves, so an artifact panned away from and back is not renamed, and the store reuses its colour map instead of rebuilding it — which is also where a held-but-not-served artifact stops a band being colour-stale. The filter bit is read from the response every time and never from the store (decision 0104). Held payloads are capped at 500,000 with the least recently *served* evicted first, never the set on screen — sized so GeoNames' 464,655-artifact rung fits, since evicting that is evicting the case the store exists for; it counts artifacts and not bytes, which is the honest bound and not the tight one | six channel tests (wholesale served set beside a held store, no renaming on a pan back, the drop on each key, the bit from the response, eviction order, the store surviving a layer switched off) and a store test that a settle naming nothing held reuses the colour map object | — | **done** 2026-08-28 (branch `client/step-hold`). ⊘ **Nothing is saved on the wire or in parsing yet**: without the affordance for *what I hold* the response still carries every payload. What is saved is naming, colouring and the GPU lookup texture. Unmeasured |
