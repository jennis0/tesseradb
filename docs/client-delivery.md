...
- **The client's artifact cache is started, and has its own map**:
  [`artifact-cache-handover.md`](artifact-cache-handover.md) — why it is the only bound a scattered
  layer has, the facts it turns on, the filter bit it needs, and the traps. Its first step is **S13
  below, done**: the filter bit is ruled and on the wire. The client half — the channel holding
  payloads across a served-set change — is next and has no row yet.
