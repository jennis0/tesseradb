...
| H3 | the draw under a highlight (§5.3): the matched marks lit and the rest at 0.22 alpha — a fifth per-point attribute through the slab's own dirty-span path, the switch a uniform; the wash reading `highlighted` under a highlight, `matched` under a filter with no highlight, `visible` under neither | `deck/test/slab.test.ts` (ones where a band carries none; the band's own bits at its slot), `deck/test/density.test.ts` (the channel is read, so a highlight and a filter are different pictures) | **done**. ⊘ `point_rows = "highlight"` is on the wire and **the client does not use it**: a highlight change re-requests in full, which is correct and costs the render columns it already held |
