...
that rung declares the two views `knn` and `pca64`, `prepare.py` runs both routes over one
embedding matrix in one process, and both clustering layers are named on both. Whole corpus:
`prepare.py` 13 m 0 s at 22.9 GB peak, `tessera build` 54.5 s to a 1.5 GB bundle carrying both
views, `verify --deep` clean.

⊘ **The fidelity criterion above is withdrawn** (owner ruling, same day): the rung is a
demonstrator and a speed benchmark, so which projection better preserves the embedding is a
question about UMAP rather than about Tessera. Two Tessera-side facts carry forward instead, and
both favour full dimension — which is why it is the anchor view. **It is 3× faster end to end**
(94 s against 280 s; reducing to 64 dimensions costs 24 s and then leaves UMAP a slower graph to
build than the card had already built in full dimension). And **the anchor layout decides how much
of a membership the tile index can bound**: in `knn` a cluster is 3.5-4.1 contiguous row runs with
two thirds of each level under an index node, in `pca64` 27-29 runs with every level `everywhere`.

⊘ Neither view reproduces bit for bit the way the rung's old seeded CPU path did — CAGRA's index
build takes no seed. See [`../test_corpora/arxiv/README.md`](../test_corpora/arxiv/README.md).
