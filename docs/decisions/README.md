...
| [0063](0063-the-inverse-permutation-is-stored-for-the-filtered-viewport.md) | 2026-08-11 | Row→entity is stored as `row-entity.u32` after all: a filtered viewport asks it per row, and the keyed bijection's ~17.5 ns is the whole cost of the cheap route |
| [0064](0064-none-of-requires-a-value-and-names-one-column.md) | 2026-08-11 | `none_of` means *carries a value, and none of these matches it* — the presence requirement is what keeps a negation positive and closes 0060's C11 oracle at the same time |
