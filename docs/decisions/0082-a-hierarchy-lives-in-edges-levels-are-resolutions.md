...
| Where | What changes |
|---|---|
| The model's hierarchy declaration | `kind = "nested"` implies a tree in edges and **no levels**; `kind = "stacked"` implies levels and **no lineage**. *(The third shape §6.2 describes — levels **and** edges, the case administrative boundaries motivate — had no value here until [decision 0087](0087-cross-level-edges-are-information-not-rollup.md) added `kind = "tiered"`. Its edges run between levels and are information rather than roll-up, so nothing in this ruling's separation of tree from levels is disturbed by it.)* |
