...
a request that fails any of them takes no effect. Beyond those, four checks carry a rule worth
knowing:
...
A row whose coordinates fall outside the view's fixed bounds refuses the request before anything
is acknowledged, written to the WAL, or allocated an entity id.
...
about the map becomes wrong at a flush. A session picks up the new rows on its next request, and
the staleness signal (Geometry and staleness, below) tells it that something has changed.

## Merge

A merge bounds how many segments, term-index layers, external-id runs and dictionary entries a flush
...
Merge publishes in two parts, on the same cadence. Artefacts that do not move a row, term-index
layers, external-id runs, dictionary entries, coalesce as one change to the manifest. Live segments
...
A pending deletion is not dropped by a merge. Its row and its term-index entries are carried into the merged
...
Compaction is the one operation that may drop a row and its term-index entries. It runs as a single pass
...
- it is the only way a partition returns to one segment, one base term-index layer, one external-id
  run and one locator. Flush and merge only ever add to those counts.

A fold takes a snapshot at the start of its run. The snapshot names which rows and term-index entries to
...
row, not its term-index entries, and not its external-id binding. An entity a flush gave a fresh row to
...
brings the retired entry back. This causes no harm. The entity it names has no row and no term-index entries
...
| Rule F | Deletion | The compaction fold that removes the item's row and its term-index entries, and nothing else | The row still exists in a segment until that fold runs. Removing the record any earlier would leave a segment reachable that still contains the item |
...
acknowledges the caller, so that a bundle can be opened without replaying the log. The manifest
takes the deletion and suppression records from the live overlay directly, never from an earlier
manifest, because copying one forward could republish an unsuppress the live overlay has already
reversed.
