...
Operators by column family: a **category** takes `eq` and `in`, whose values are the vocabulary's **key (a JSON string) or its code (a JSON integer)**, freely mixed — the two are unambiguous because a key is never an integer. A **`utf8`** column takes `eq`, `in`, `prefix` and `contains` against the stored bytes — `in` being `eq` over a list, which is not a category-only generalisation. A **`text`** column takes `match` against analysed tokens (⊘ unbuilt, [#44]). `/v1/meta`'s `filter_operands` publishes the per-column operator list, so a client need not infer it.
