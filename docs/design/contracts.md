...
**`declared_scalars` is the column schema in full** *(r25)*: `[{name, arrow_type, category?}]`, where `category` is `{vocabulary, kind, listing}` for a category column and **absent** for a plain one. Its absence is the signal — the hot path ships a category as a bare integer of the declared width, so without this a client cannot tell a `u16` category from a `u16` integer, and a legend has nothing to key off. `kind` and `listing` are read from the vocabulary the column names, so two columns sharing a value set agree on both by construction (§3.9 refuses a disagreement at parse).
...
- **`listing = "public"`** serves the value set as authored to any principal with a session — an authored assertion that these names disclose nothing (§3.8). **⊘ `listing = "per_viewer"` is refused**, `500 fail-closed`, naming the column: filtering per principal needs §3.3's per-`(column, code)` entity-space membership set, which no build emits. Refused rather than served empty, because an empty value set is a *real* answer — it is what a principal who may see none of these values is told — so returning it for an unbuilt predicate would make the two indistinguishable, and a viewer would render a computed-looking blank legend. Serving it unfiltered is the C11 disclosure itself.

Session-bearer authenticated like every other route on this plane, and **not behind the compute-admission gate**: the work is a bounded walk of an in-memory map with no mask composition, no projection and no file IO — the same class of request as `/v1/meta`.
...
1. *tiles*: `(tile: uint64, visible: uint64, matched: uint64, served: uint64)` — tile per 2.5's prefix encoding at depth `zoom`; exact masked counts. `visible` is the **composed** count — how many of this tile's items the principal may see, which a filter never changes — and `matched` how many of those the filter admits; they are equal with no filters (§8.1). A client showing "X of Y" reads `matched` over `visible`, and must not read a fall in `matched` as a permission change. **`served` is appended last and its position is contract** *(r7)*: decoders that index this batch positionally exist, so inserting it earlier would silently rebind `visible`/`matched`.
...
`filters`: a **boolean expression over declared columns** *(r26, decision 0059)*. A node is a leaf — one column name mapped to one operator — or a combinator, `all_of` or `any_of`, over sub-expressions:
...
Operators by column family: a **category** takes `eq` and `in`, whose values are the vocabulary's **key (a JSON string) or its code (a JSON integer)**, freely mixed — the two are unambiguous because a key is never an integer. A **`utf8`** column takes `eq`, `prefix` and `contains` against the stored bytes. A **`text`** column takes `match` against analysed tokens (⊘ unbuilt, [#44]). `/v1/meta`'s `filter_operands` publishes the per-column operator list, so a client need not infer it.

**An unknown column name is `422`; an unknown *value* is an empty operand.** The distinction is the disclosure control: a caller naming a column that does not exist made an error, but refusing an unknown *value* would make the filter an existence oracle over exactly the vocabulary `listing = "per_viewer"` hides. The two must not be conflated, and a client must render "no matches" identically for a value it may not see and one that does not exist.

**Empty combinators are their operators' identities and differ**: `all_of: []` matches the whole candidate, `any_of: []` matches nothing. **Nesting is bounded** by the deployment's limit and an over-deep expression is **refused, never flattened** — a flattened expression answers a different question. `none_of` is **not accepted**: it is specified and unbuilt (decision 0059), so naming it is `422` rather than an ignored clause.
...
**`listing` stops being ⊘ and becomes typed.** `public` publishes; `per_viewer` is refused
`500 fail-closed` pending §3.3's membership sets — refused rather than served empty, because an
empty value set is a real answer and conflating the two would render an uncomputed blank legend as
though it had been computed. The field changes from a string to a two-variant enum on `arrow_type`'s
