...
- **The mask.** Every count, hull, centroid, threshold and candidacy test is a function of
...
- **A deleted member never returns to the right operand.** While the delete is pending, the member
  ...
  no mask ever again — and **I9 guarantees no future point can occupy its ID** (decision 0047 states
  this consequence directly: the dead entity's ID stays burned, *"and every mask or generating set
  holding the old ID stays valid"*). **I9 has since been relaxed — a slot is reused after a fold
  ([decision 0072](../decisions/0072-entity-ids-are-slots-and-are-reused-after-a-fold.md))** — and
  what carries this bullet instead is that decision's reconciliation rule: a slot returns to the
  allocator only once every durable entity-space structure naming it has been dealt with in the same
  fold, and `G` is one of those structures. The fold already computes the list (spec §4.2); it must
  act on it. Containment fails
...
set are declared, and edited, together** (rep §5.0.1's rule, kept) — the API refuses a `G` edit
that does not accompany a content edit, so re-basing a declaration is at least a deliberate act.
...
construction), **withdraw content**, **degrade artifact** (to its derived content, model §8.6),
**refuse the write**, **notify the caller**. No cell deletes an artifact: a point event never
destroys a caller's object.

### 3.1 Point event × membership source — what happens to membership, counts and the threshold

| Event | Enumerated | Spatial predicate | Attribute predicate |
|---|---|---|---|
| **Ingest** | **nothing** — membership is frozen at declaration (I8's shape); the point is in no artifact until the layer regenerates. Safe: counts only ever *understate* what regeneration would show — fail-closed, stale-not-unsafe (rep §2.2.1) | **nothing stored; the point is a member at its flush** — membership is derived per request from the geometry, so the row's arrival is the whole event (rep §2.0). Earlier than flush is impossible: counts are row-space and the point has no row | as spatial: the value column's flush extents carry it; the filter machinery answers per request ([`filter-index.md`](filter-index.md)) |
| **Delete** | at accept: the entity leaves every composed mask, so every masked count, hull and threshold is correct with no artifact work. At the fold: the row form drops the member's bit in translation (spec §4.1) | at accept: the row leaves `denied[slice]`'s complement; `range_cardinality` over the mask is correct immediately. At the fold: rows renumber and the ranges re-derive per request — nothing stored, nothing stale | at accept: composed verdict excludes it (filter-index §6.1). At the fold: the attribute pass blanks the slot (filter-index §6.2). All existing machinery |
| **Suppress** | **nothing artifact-side** — the member leaves the composed mask at accept; counts fall, hulls recompute without it, an artifact may drop below threshold and vanish. Rule S: no stored structure changes, postings and membership untouched | same — the mask is the only operand that moves | same |
| **Unsuppress** | **nothing** — the member returns to the composed mask at accept; counts restore. Reappearance above a *cached* threshold decision lags fail-closed (spec §4.4) | same | same |
| **Update** (delete + re-ingest) | the union of the two rows: the old life leaves masks at accept and leaves the row form at the fold; the new life is a new entity, in no enumerated membership until regeneration. **A caller must expect enumerated counts to drift down under churn** — regeneration is the repair | the old row leaves, the new life's row is inside or outside the shape on its own coordinates from its flush — a point that moved across a boundary changes membership correctly, no artifact work | same, via the new value |
...
- **Row-form translation is member-wise, never range-wise** (spec §4.1). A range translation of a
  ...
  count could lift an artifact across `min_visible_members`, weakening C1's control. The
...
### 3.3 Point event × gate mode — what happens to existence

| Event | Derived threshold | Substitutive | Conjunctive |
|---|---|---|---|
| **Ingest** | enumerated: no movement (count frozen until regeneration). Predicate: the count may cross the threshold at the point's flush and the artifact appears — the gate meaning what it says about a corpus that genuinely grew | **nothing** — existence never derives from the corpus; the count beside it moves with the mask and may be 0 (model §8.5) | as derived for the threshold half |
| **Delete / suppress** of members | the live count falls at accept; an artifact below threshold is **omitted at the next request** (spec §4.4's live-threshold rule) | **nothing** — the artifact's own terms alone decide; the count beside it falls at accept | both: the threshold half as derived |
| **Unsuppress** | count recovers at accept; reappearance above a cached candidacy lags fail-closed (spec §4.4) | nothing | as derived |
...
*membership* losses for the caller's regeneration planning.
...
### 4.4 The threshold is enforced on the live count; caches are candidates, never authorities
...
> serve anyway, and enforces `min_visible_members` on **that** count. `verdict` on the artifact's
...
*recovered* (unsuppress) or whose threshold a candidacy snapshot missed stays absent until the
...
| **Regenerate** | create successor, then drop predecessor — never 10⁷ denies through a lane sized for trickle (rep §5). **Not atomic, deliberately**: if the drop fails after the create, both generations serve — each individually gated and sound, so the intermediate state is duplication, never disclosure, and the caller retries the drop. The reverse order has an outage window and is not used | the two registry records | the suppression-carry refusal (rep §5.0.2: live suppressions must arrive absent or already suppressed, by stable key); the dangling-dependent refusal (rep §5.0.4) |
...
| **Level lifecycle ops** | **none exist.** Lifecycle is a layer property (model §2.1); a level is replaced by regenerating its layer | — | a per-level suppress or drop is refused as an unknown operation |
| **Gate-mode change on a populated layer** | narrowing (adding the threshold): in place, version bump. Widening (derived → substitutive switches `min_visible_members` off): **the suppress–edit–unsuppress path, layer-level** — the same decomposition as an artifact gate narrowing, because the fail-open direction here is the widening | WAL'd registry edit | in-place widening is refused; the register row model §4.2 demands accompanies the mechanism |
...
**Why regeneration is refused rather than repaired, in both refusal cases:** replaying suppressions
or repointing edges automatically fails open exactly when the caller reshaped the layer — a missing
key, a renamed artifact — and each such failure silently un-hides something an owner hid, or
attaches a label to a cluster it was not generated from (I8's class). A refusal makes the operator
perform the reconciliation the service cannot verify. (Rep §5.0.2 and §5.0.4 carry the full
arguments; this table only inherits them.)
...
it meant. What is newly stated for the **viewer**: content vanishing is a signal that a member was
denied, and it is bounded to viewers who satisfied `G` — principals who could already see every
member — so it is C17's accepted shape (a delete/suppress signal on items the principal already
sees), not a new channel. Recommended as an annotation on C17 rather than a row; register deltas
below.
...
staleness, if the resolved-set route ships as drafted (a below-threshold artifact remains
...
undeclared content — is standing business (rep §12) and is neither worsened nor resolved here:
permanent unsatisfiability changes *for how long* content is withheld, not the distinguishability
mechanics.
...
- **`G` storage** *(assumed)*: entity-space small sets, stored as sorted arrays beside the content
  ...
  Roaring), mmapped and touched only when the truth is needed — regeneration, the fold, an audit —
...
- ✔ **Layers take an entity ID at create so that layer suppression rides the deny lane** (spec §6) —
  **ruled, accepted** *(owner, 2026-08-15)*. An operator can hide a leaking layer immediately and
  ...
  layout nor uniformity but **budget**: entity IDs are `u32` and never reused (I9, contracts §1),
  while a regenerated 10⁷-artifact layer mints 10⁷ fresh IDs per regeneration (rep §5.2, which
  already records that any fixed block exhausts). Daily regeneration spends the space in about a
  ...
  the question to put to it is what the entity budget is under I9, not where type bits go.**
...
| rep §12, third ruling (freeze `\|G\|`) | **withdrawn**, replaced by spec §9's first ruling |
...
- **Drill-down by identifier under the live-threshold rule** (spec §4.4): the resolved-set lookup
