...
**Status:** Draft for review — revision 25
...
5. Write frozen-format, cache so eviction rebuilds rather than fails, and return an opaque token.
...
> **⊘ Specified, not implemented (I13b only).** There is one partition, no required-set gate, no reachability computation and no cross-partition containment, so nothing exercises this half and no test covers it. It is *safe today* only because a single partition makes the unreachable case unreachable — not because the rule is enforced. A reader must not read I13a's coverage as evidence for I13b, and the first deployment to define a second partition acquires an unimplemented invariant.
...
For the label half of that obligation an oracle is available rather than needing to be written. Where the data function parses an access expression in the syntax of Appendix E, the reference is **`accumulo-access`** — a spec-backed, zero-dependency, long-hardened implementation of exactly those semantics. Generate random expressions and random authorisation sets, evaluate in both, assert agreement. This would live entirely in the test suite; the JVM it requires belongs in CI and never in a deployed process.

> **⊘ Specified, not implemented.** No differential oracle exists: there is no `accumulo-access` dependency, no access-expression parser, no k-of-N evaluation and no CI to run any of it. The only authorisation plugin built is a **passthrough** — an item's terms are the descriptors the caller supplied, verbatim — for which **I5** is trivially true and therefore untestable. **This matters more than a missing test.** §2.4 names the two-function consistency obligation as the single largest unverifiable dependency in the design; the differential oracle is the whole of the proposed mitigation, and until it exists that dependency is unmitigated, not mitigated. A deployment that ships a real plugin before the oracle ships is running the design's largest known risk with nothing checking it.
...
> **⊘ Specified, not implemented.** No semi-join is performed anywhere. The exploded pair relation is an *optional* build input (`pairs.parquet`), off by default; postings are built by a streaming pipeline that spills banded pair files to disk, scatters through per-term cursors and encodes Roaring bitmaps in parallel. What survives of the argument above is the part that matters and is unaffected: **the array-containment formulation is refused**, and the pair relation — however it is joined — is the schema the build works from. What is not true is that a semi-join is the mechanism, or that the pair file is required. The build's actual requirement is being memory-bounded: the in-memory linear build was OOM-killed at 10⁹ on a 47 GiB box and survives only as a byte-identity oracle for the streaming one.
...
**Measured latency, and the cost model it implies.** Warm, single segment, 10⁹ items, ~300-tile viewport (`docs/evidence/memos/2026-07-30-viewport-hot-path-and-bundle-size-review.md` §1.3):

| | p50 | p99 |
|---|---|---|
| *k* = 50 | 135.1 ms | 158.0 ms |
| *k* = 1000 | 163.6 ms | 191.3 ms |

Selection is **83.2–88.8%** of that; gather is 5–6%, counting 3–6%, composition ≈0%. **Latency is hundreds of milliseconds at 10⁹, not single-digit milliseconds**, and an earlier claim of the latter in this section was written against a cost model the implemented route does not have.
