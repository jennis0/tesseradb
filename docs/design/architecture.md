...
**Status:** Draft for review — revision 53
...
| C27 | Roster ordinal gaps under a gated view | A group's ordinals are monotone and served in roster order, and a gate-failed view is omitted — so a principal seeing ordinals 0, 1, 3 learns *a* view exists (or existed) at #2 that they may not reach, and a moving ordinal high-water counts hidden creations over time. Reachable since the gate landed (2026-08-31); a dropped key leaves the same gap, so the two are indistinguishable | Low | **Accepted, not mitigated** ([decision 0110](../decisions/0110-the-ordinal-gap-is-accepted.md)): a gap says a view exists, never whose data or what it holds — no entity, count, membership or key is disclosed, and the gate's own closure keeps the view's *name* out of every surface. A deployment whose roster shape is itself sensitive gates the **group**, which hides the whole roster, gaps included. The alternatives — per-principal-dense ordinals, or serving none — re-open the per-session handle machinery decision 0006 retired, to close a channel of one bit per creation. C15's family: knowing something was created is not a security leak (owner ruling, 2026-08-30) | Accepted |
