...
**Status:** Draft for review — revision 61
...
**I8 — withdrawn 2026-09-07 ([decision 0135](../decisions/0135-a-generating-set-is-the-callers-claim-i8-withdrawn.md)).** It read: *a label's generating set is immutable once supplied.* A generating set is the caller's claim about their own data, changeable at ingest in either direction; containment is evaluated against the set as declared, inside the mask (I2, I3), and the declared set is trusted as C12 already says. The number is kept so citations resolve.
...
- **`terms_of_labels([label]) -> [term descriptor]`** — the list form, run once per item at both entry points: the build's list column and `/control/ingest`'s `access` list each carry one label per element, and the plugin returns exactly one descriptor per element, in order. Must be fast.
...
- **`present_terms([term descriptor]) -> [display label]`** — the *presentation* side, run once per item drill-down over the descriptors that survived the intersection with the asking session's satisfied set (§7.4, [decision 0114](../decisions/0114-the-drill-down-serves-the-satisfied-labels-only.md)). One string per descriptor, in order. It decides only how a label is **spelled**: which labels reach it is settled before the call, by an intersection the plugin has no part in, so no implementation of this function can widen a disclosure. ⊘ **The routing is the built-in plugin's**, there being no wasmtime host (**I6**); `builtin:passthrough`'s descriptors *are* the caller's own label strings, so its implementation is the identity — which is its correct answer and not a stopgap.
...
**A drill-down names the item's labels, and only the ones this principal holds** *(2026-08-31, [decision 0114](../decisions/0114-the-drill-down-serves-the-satisfied-labels-only.md); C30)*. The response carries the intersection of the item's own term set with the session's satisfied set, spelled by the plugin's `present_terms` (§6.1) and sorted. The full label set is **not** served and was declined: a viewer learning that an item they can see also carries a compartment they do not hold is a disclosure about the corpus's labelling, not a filtered view of it. The intersection is what makes every string computable from inside `M_auth` — the same test **I2** applies to a count applies to a name — and it is enforced by construction rather than by a filter: the only descriptors the serving path holds are the ones the session's own credential presented plus `public`, so there is no descriptor in scope for a term outside the grant. The bundle stores the entity→term transpose the intersection reads (`contracts.md` §2.4); it is server-side, and the full set it holds is read on the write path alone, to refuse a re-label carried in on a second view's row (`views.md` §4).

### 7.5 Cluster visibility

**A supplied hierarchy is a tree in its edges, and every node is tested on its own** *(r43; this replaces the root-down descent this section specified — decisions [0080](../decisions/0080-the-frontier-is-a-per-artifact-test.md), 0082, 0083; the mechanism is [`annotation-representation.md`](annotation-representation.md) §6.2–§6.3)*. Store a membership bitmap per node, evaluate `and_cardinality` against the mask per node, and serve the node iff its masked count clears the declared **existence criterion**. The descent, and the frontier defined as its stopping point, are withdrawn: a real hierarchy does not cover — HDBSCAN's children are subsets of their parents but do not exhaust them, 20–25% of points falling out as noise at each split on the corpus measured — so a walk that stops at a node abandons members that node's children never held, and a condensed tree is unbalanced, so a level number says nothing about position in the lineage.
...
- *Adding newly ingested items to a generating set* — was forbidden by I8; permitted since decision 0135 as the caller's own change to their claim (§7.6).
...
| C29 *(r44; widened r57)* | An artifact's parent identifiers on the wire | The artifacts frame carries a served artifact's **parents** — a list, since r57, because a `dag` layer's artifact has several and a tree's has at most one; every clause below holds per entry, and the list's length is the number of served parents, all of them in the response — which is the structure a client needs to nest what it draws or to filter to one subtree while still drawing the rest of the map. It is a relation between two artifacts, so it says that one grouping contains another — a fact about the *shape of the analysis*, which for a clustering is corpus-derived rather than authored | Low | **Named only where the parent is in the same response**, which is the whole control and is structural rather than a check that could be forgotten: the field is resolved against the response's own membership after it is settled, so there is no path by which an unserved artifact's identifier reaches it. A parent that exists and was withheld — below its own criterion for this viewer, suppressed, or dropped by the layer's frontier — is **null, identically to a root**, so the two are one value on the wire and the register's usual rule holds: an artifact withheld is indistinguishable from one that never existed. What a viewer learns is therefore a relation between two artifacts they were already served, each of which passed its own criterion against their own mask — no membership, no ordinal (C8), no unmasked quantity, and nothing about an artifact they cannot see. The drill-down route carries `null` unconditionally, its response being one artifact with nothing to name. **The residual is that lineage is corpus-derived where the clustering is**: a viewer served two clusters learns the algorithm split one from the other, which they could not otherwise infer — accepted because both artifacts were already disclosed to them in full, and the relation adds no member and no count | Accepted — bounded to the response's own membership |
| C30 *(r55; widened r56)* | An item's **satisfied** labels, its **reachable views and their positions**, and its **scoped values**, on the drill-down | `POST /v1/items/{tessera_id}` names the labels the item carries **that this principal satisfies** — the intersection of its term set with the session's, spelled by the plugin (§6.1, §7.4). A viewer therefore learns which of their own grants admits them to this item, where before they knew only that one did | Low | **The intersection is the control, and it is structural** ([decision 0114](../decisions/0114-the-drill-down-serves-the-satisfied-labels-only.md)). Every string served is a descriptor the session's own credential presented, **plus `public`** — the label every principal holds, added inside the trust boundary at authorise (§6.1) and therefore in every session's satisfied set whatever the credential said. The serving path holds no other, there being no ordinal→descriptor route that does not pass through the session, so the response is computable from inside `M_auth` and **I2** is met by construction rather than by a filter that could be forgotten. What is *not* served is the item's full label set: option B at the ruling, declined, because a compartment the viewer does not hold is a fact about the corpus's labelling and not a view of it. The residual is real and small: a principal holding two grants learns which one this item answers to, which is a fact about an item they can already read in full, and it says nothing about any item they cannot see — a masked or suppressed item is the same `404` it was, body for body. Nothing about the item's *other* labels, their number, or their existence is recoverable: the array's length is the size of the intersection, which the principal's own grants and this item's visibility already bound. **Widened at r56** (owner ruling 2026-09-01) to the two fields beside the labels, on one argument: everything this endpoint serves about an item is computable from inside `M_auth` **and the views this principal may already reach**. `views` lists only the views the session's gate admits — resolved once at authorise, so a gate-failed view costs the same lookup as a name nobody declared — and for each of those, membership is `mask ∩ members(view)`, exactly what a viewport of that view serves (`views.md` §9's equality). A **position** is the quantity that viewport already ships for that point, in the same grid units, deinterleaved. A **scoped value** is this item's own, read at an entity already established visible out of a column indexed by entity id: no aggregate, and no quantity derived from anything outside the mask. A key is served where the principal may reach *a* view holding it — its own group's or a sharing group's — so the key set is the one their own roster gives them. What is **not** served is anything about a view the gate refuses: not its id, not its position, not a value under its key; a point held only in such views is served `[]`, which says *none of this item's views is one you may reach* and nothing further — a point in no view at all is a `404` before the array is built. The residual is unchanged in kind: which of a principal's own reachable views hold an item they can already read in full | Accepted — the intersection is the disclosure, and it is the principal's own; the views and values are the principal's own too |
| C31 *(r58)* | Listing and suggestion time on a `derived` category column | Both `/v1/categories` and `/v1/categories/{column}/suggest` answer the visibility predicate by probing one memory-mapped posting per value walked, so service time tracks **how many values are walked** — hidden ones included. On the enumeration, which walks the whole vocabulary, that is a coarse read of its size. On the suggestion verb the caller chooses the range: response time is a function of how many values sit under the prefix they typed, pre-mask, at a resolution the whole-set walk never offered. **The same quantity reaches the wire as one bit**: `more` on a spent walk budget says at least `max_suggestion_walk` values sit under that prefix, and a walk that spends its budget has already taken the budget's time. Measured at 10⁷ values over 10⁸ entities (`probes/2026-09-02-value-suggestion/`): a hidden value costs **0.06–0.13 µs** at the median whatever its member count — 3.4 µs p99, 103 µs worst — so the quantity carried is the value *count*, not the values' sizes. ⊘ Not measured over a network, and not at 10⁹ | Low | **The quantity is a coarse count of values under a prefix the caller typed** — never a value's name, never its member count, never membership or which items, and nothing about another principal's `M_auth`. The **route** is a function of the **declaration** and of the request's prefix, identical for every principal, never of a statistic or of how much the principal can see (§8.2); what varies with the principal's own mask is only where the walk *stops*, which is a fact about their own visible set that the answer's length already gives them. The **budget bounds one request's work and not the channel** — the enumeration walks the whole vocabulary unbudgeted and carries the same fact more slowly. Comparable systems carry the identical channel ambient and unregistered, and here it is registered, bounded and conscious — decision 0067's reasoning, applied to a value set rather than a term set. **The route that narrows it is built and measured** *(r61)* — a per-session set of visible values reads no posting, so a hidden value under the prefix costs a bit test rather than a probe: 3.4–3.8 ms against 41–45 ms for a viewer who sees nothing under a broad one-character prefix. It does not *close* the channel, because the walk over the prefix's entries remains; the route that would close it — a set over the suggestion index's payload records, at two to four times the bytes per session — is priced and held in reserve (`value-suggestion.md` §6.3). Both surfaces are built (2026-09-02); the channel's constant on the shipped probe route is 36–50 ms for the sparsest viewer's spent budget at 10⁷ values, a 4.2 MB in-memory bucket table over the code's high bits having taken most of the record search back (`value-suggestion.md` §6.2), the enumeration carrying the weaker whole-set form. **The suggestion verb's route may follow the principal's own cardinality** *(r60, decision 0124)* — the per-session set is built where that cardinality is at or under `selection.max_suggest_set_entities`, which is §8.2's one exception and admissible because the quantity consulted is the caller's own, already served as `visible` by a zoom-0 viewport | Accepted — owner ruling 2026-09-02; **narrowed** for indexed columns once the per-session set is warm (decision 0124): no posting is read, the walk over entries remains. Unnarrowed on the probe route — the first keystrokes on a session-column pair, viewers wider than the constant, a value minted since the sweep, blob-resident columns, and the enumeration. *(0124 and r60 read this as closed, on a set walk over positions inside the prefix range that is not implementable; `value-suggestion.md` r5 §6.3 carries the correction)* |
| C32 *(r59)* | A highlight's counts and bits — `highlighted` per tile, per served point and per served artifact (`highlight-and-hierarchy.md` §2) | Per tile, per served point and per served artifact, whether the request's second expression holds — a second filter's answer beside the first, over the same masked candidate | Low | **A second evaluation of the filter contract over the same masked candidate**, disclosing what one request carrying `all_of[filters, highlight]` in `filters` would have disclosed, differently arranged: `highlighted` per tile is the `matched` count that request would carry, and the per-point bit is set on points already served. The per-artifact bit is decision 0104's bit under the conjunction. Nothing derived from outside `M_auth` and no quantity a caller could not already obtain in two requests. `point_rows = "highlight"` is a column subset of the full answer to the same request, on `artifact_rows`' argument (contracts r44). ⊘ **Not built** | Accepted — a second filter over the same candidate (⊘ not built) |
| C33 *(r59)* | The browse verb — a layer's artifacts by lineage (`highlight-and-hierarchy.md` §4) | A layer's artifacts by relation — existence, masked count, filtered count, name and parents — without a viewport and without a budget, so a principal can enumerate every artifact of a layer that passes their criterion | Low | **Enumeration was already available**: `artifact_budget` is a request bound and never a disclosure control (contracts §3.2 r38), so a zoom-0 viewport at a large budget with every level named serves the same set; this verb serves it paged and by relation instead of by position, and the gate runs before the page so a withheld artifact leaves no countable gap. Each row is the artifacts frame's identity row plus a name the principal may read, and each relation is C29's, named only where both ends passed their own criterion. The counts are C8's `and_cardinality`, per request — the filtered one against a verdict computed inside `M_auth` by the filter contract's own routes. **The residual is the search form**: a substring over keys and names this principal may read — an existence test over artifacts already servable to them, and nothing about one that is not; an artifact below its floor matches no query. The `member_of` leaf's empty operand has a timing residual of the C4/C24–C26 family — a withheld artifact's criterion is evaluated where an unknown identifier is a lookup miss — registered here rather than as a row. ⊘ **Not built** | Accepted — bounded to what a viewport already serves (⊘ not built) |
...
- **r61** — **C31 is narrowed rather than closed where the suggestion set is warm** (2026-09-02; [`value-suggestion.md`](value-suggestion.md) r5 §6.3, §8). Both of r60's unbuilt items are built: §6.2's 4.2 MB bucket table over a code's high bits, which takes the sparsest viewer's spent walk budget at 10⁷ values from 68–80 ms to 36–50 ms, and §6.3's per-session set. r60 recorded the set as **closing** the row, on a keystroke that iterates the set's bits inside the prefix range — but a prefix range is over entry strings and a value's dense position is its rank in key order, so no such iteration exists. The set route walks the same entries and tests a bit rather than a posting: an order of magnitude on the channel's constant (3.4–3.8 ms against 41–45 ms for a viewer seeing nothing under a broad prefix), not a closure. What would close it exactly — a set over the suggestion index's payload records, at two to four times the bytes per session — is priced and declined under memory-first. C31's disposition and mitigation are amended; no new row, and no invariant moves. [Decision 0124](../decisions/0124-the-suggestion-route-may-follow-the-viewers-cardinality.md) is settled and says *closed*; the correction is recorded here rather than by amending it.
- **r60** — **the suggestion route may follow the viewer's own cardinality** (2026-09-02, **owner ruling**; [decision 0124](../decisions/0124-the-suggestion-route-may-follow-the-viewers-cardinality.md), [`value-suggestion.md`](value-suggestion.md) §6.3, §8). §8.2's *keep it a pipeline, not a planner* gains one narrow exception: category value suggestion builds a per-session set of visible values instead of probing per value where the principal's own composed cardinality is at or under `selection.max_suggest_set_entities`. The rule's reason — a route that makes execution time a function of how much the principal can see — does not reach a route keyed on the caller's *own* cardinality, which a zoom-0 viewport already returns as `visible`. **C31's disposition is amended** rather than reversed: still accepted, and closed for an indexed column once that set is warm. No new row, and no invariant moves..
- **r59** — **a highlight's counts and bits, and the browse verb, are registered** (2026-09-02, `highlight-and-hierarchy.md` r5): C32, a second filter expression answered per tile, per served point and per served artifact over the same masked candidate; C33, a layer's artifacts served by lineage without a viewport, enumeration having been available under `artifact_budget` already. Both ⊘ not built.
- **r58** — **listing and suggestion time on a `derived` category column is registered** (2026-09-02,
  **owner ruling**; [`value-suggestion.md`](value-suggestion.md) §8, decision
  [0121](../decisions/0121-the-suggestion-walk-probes-per-request-and-its-timing-is-accepted.md)).
  **C31** is new: both `/v1/categories` and the suggestion verb probe one posting per value walked,
  so their time is a coarse count of the values under what the caller asked for, hidden ones
  included, and `more` on a spent walk budget puts one bit of it on the wire. Registered rather than
  closed, on decision 0067's reasoning for term postings; the lever that would close it is designed
  and priced. **C8's cell is amended** — the suggestion verb serves its quantity on request, per
  request and never as an order — and **C24 gains a pointer** to the new row, the two being different
  quantities on the same postings. No invariant moves.
- **r57** — **an artifact names every served parent** (2026-09-01, **owner ruling**; decision 0117,
  `contracts.md` §3.2 r71). C29 is widened from one parent identifier to a list, its control
  unchanged and applied per entry; no other row moves. The same ruling settled that a withheld
  artifact is not in the viewer's tree (`annotations.md` §6 r9) — a correction to the cut, not a
  register row.
- **r56** — **the drill-down is the whole permitted picture of one point** (2026-09-01, **owner
  ruling**; `contracts.md` §3.2 r68, `views.md` §5–§6 r27). `POST /v1/items/{tessera_id}` gains the
  views the item is in **that this principal may reach**, each with that view's own position for
  it, and the group-scoped attribute values keyed by the owning group's key. **C30 is widened
  rather than joined**: it is one row about one endpoint — what that endpoint may say about an item
  the principal can already read in full — and the two new fields are inside the same bound. The
  argument is per field. A **position** is the quantity the viewport already ships for that point,
  in the same grid units. **Membership in a reachable view** is `mask ∩ members(view)`, which the
  viewport serves as an equality rather than an inclusion (`views.md` §9), so it is derivable
  already. A **scoped value** is this item's own, read at an entity established visible from a
  column indexed by entity id — no aggregate, and nothing outside `M_auth`. And the gate is prior
  to all three: both fields are built from the session's resolved visible-view set, so a
  gate-failed view is named on neither, and a point held only in such views serves exactly what it
  served before. No invariant moves, no new verb, and nothing here changes the `404`, which is
  reached before either field is built.
- **r55** — **the drill-down names an item's satisfied labels** (2026-08-31, **owner ruling**,
  [decision 0114](../decisions/0114-the-drill-down-serves-the-satisfied-labels-only.md)). §6.1's
  plugin boundary gains a fourth function, `present_terms`, which decides how a label is *spelled*
  and never which labels are served — the intersection is settled before the call, so no plugin
  implementation can widen a disclosure through it; `builtin:passthrough`'s implementation is the
  identity, and that is its answer rather than a fallback, its descriptors being the caller's own
  label strings. §7.4 states the rule: the response carries the item's terms intersected with the
  session's satisfied set, sorted, and never the full label set — option B, declined, because a
  compartment the viewer does not hold is a fact about the corpus's labelling rather than a view of
  it. **C30 is added**, and the residual it accepts is which of a principal's *own* grants admits
  them to an item they can already read in full. What makes the row narrow is that the rule is
  structural: the serving path holds only the descriptors the session's credential presented and
  `public`, so
  there is no ordinal→descriptor route by which a term outside the grant could acquire a name. The
  transpose the intersection reads is a new bundle artefact (`contracts.md` §2.4), server-side, and
  the full set it carries is read on the write path alone — where it makes `views.md` §4's label
  arm exact past a flush, closing a ⊘ that had been accepted for want of an entity→label oracle.
- **r54** — **the ordinal gap's row is deleted, the ordinal having been removed** (2026-08-31,
  **owner ruling**, decision 0113,
  superseding 0110). A view of a group is
