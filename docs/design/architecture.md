...
**Status:** Draft for review — revision 28
...
> **⊘ Specified, not implemented.** The **I5 plugin-consistency oracle** described in this section does not exist: there is no `accumulo-access` dependency, no access-expression parser, no k-of-N evaluation and no CI to run any of it. This is a specific absence, not a general one — the corpus does have a differential oracle, `reference/oracle/`, the independent Python definitions oracle that is the second implementation of record for §7.2 and the rest of the definitions, and it is what found the θ-anchor ambiguity. What it does not do, and was never built to do, is check the caller's two authorisation functions against each other. The only authorisation plugin built is a **passthrough** — an item's terms are the descriptors the caller supplied, verbatim — for which **I5** is trivially true and therefore untestable. **This matters more than a missing test.** §2.4 names the two-function consistency obligation as the single largest unverifiable dependency in the design; the differential oracle is the whole of the proposed mitigation, and until it exists that dependency is unmitigated, not mitigated. A deployment that ships a real plugin before the oracle ships is running the design's largest known risk with nothing checking it.
