...
| Amend a value's title | Safe, no rebuild — control-plane upsert (§5). `PATCH /control/vocabularies/{name}/values` replaces the title held under a key and answers how many it changed |
...
`PUT /control/vocabularies/{name}` declares a vocabulary at a running service and
`PATCH /control/vocabularies/{name}/values` pages its values (`ingest.md` §1.3, built 2026-09-08).
**A value is addressed by its key, and its title upserts while its identity does not.** A title
supplied for a key the deployment holds replaces the title it holds, and the answer says how many
titles changed. The key-to-code binding is immutable and a code is never reused, so a row already
carrying the code means what it meant; a page that would change a binding, and a declaration that
would change the vocabulary's width, value set, visibility or `reserved` list, are each a `409`.
Renaming a value for display is therefore not a rebuild; changing its width, its code or its key
still is.
