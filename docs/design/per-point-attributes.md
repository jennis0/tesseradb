...
| Amend a value's properties | Safe, no rebuild — control-plane upsert (§5). ⊘ **The route refuses an amendment today**: `PATCH /control/vocabularies/{name}/values` fills a property this deployment does not hold and answers `409` to one it holds differently (2026-09-08). Whether the upsert this row promises is built, or this row is withdrawn, is with the owner |
...
⊘ **The routes are built and the upsert is not** (2026-09-08; `ingest.md` §1.3, track T5).
`PUT /control/vocabularies/{name}` declares a vocabulary at a running service and
`PATCH /control/vocabularies/{name}/values` pages its values, so a value set grows without a
rebuild. What a page does *not* do is amend a property it already holds: it fills a title this
deployment does not hold and answers `409` to one it holds differently, on the monotone rule
(`ingest.md` §1.1). Recolouring a legend therefore still touches the build path, which is what the
paragraph above promises it would not. Whether the upsert is built or the promise is withdrawn is
with the owner.
