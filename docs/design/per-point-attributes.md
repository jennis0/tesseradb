...
> ⊘ The membership sets this waits on now **exist** — a `per_viewer` category gets its postings
> whatever its `used_for` says — so what remains is engine work rather than an absent artefact.
> Until it lands, `per_viewer` is **refused** rather than served, since serving it empty would be
> indistinguishable
