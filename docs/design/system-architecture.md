...
> **⊘ Specified, not implemented.** There is no `wal_retention` knob and no time-bounded window. What removes entries is reclamation at rotation. `Wal::rotate` writes the overlay snapshot into the new member, then deletes every member lying wholly below the lowest position anything still needs: the oldest buffered row that has no geometry yet, or the oldest artifact membership publication, whichever is lower. Rotation runs at a flush publication, at the compaction fold, and at a tick that has nothing to flush and finds the log longer than the last rotation left it, so steady-state retention is two members and nothing a replay of the live prefix would need has been discarded. Rollback recovers less than the paragraph above claims: a batch already consumed into the newer prefix lies below the reclaim bound and goes at the next rotation, so only batches not yet published survive to replay into a restored prefix. `wal_hard_limit_bytes` bounds a startup relation and nothing compares the live log against it; the log's size and its rotation bound are published on `/control/status` (write-path §1.3).
...
A module exporting `terms_of_labels`, `terms_of_auth` and `declared_bounds` — the data side once, for a list of labels the caller has already separated (a build's source column, the wire's `access` list, a view gate's `visibility`), obliged to return one descriptor per element in order (contracts §4.3). Descriptors are opaque byte strings; the engine owns interning in the single bundle-level namespace. The module hash keys both blast radii: the auth-function hash enters the fragment cache key, the data-function hash enters the manifest.
