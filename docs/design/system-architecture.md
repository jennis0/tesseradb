...
> **⊘ Only Rule S exists.** There is no compaction fold, so no deletion retires. Fail-*closed* — an entry that never retires never re-exposes anything — but it is not the mechanism, and the prohibition above is what carries the rule until the fold exists.
