...
- Refresh is **lazy and per fragment**: on next use past a staleness threshold, or when `L`'s cost bound trips, the engine builds stamp *n+1* as `fragment@n ∪ delta-postings(satisfied terms, stamps n+1…)`. One OR per delta per live fragment, amortised by laziness so a flush never stampedes the cache. The old stamp stays frozen for draining pins.
