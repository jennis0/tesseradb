...
bundle/
  ...
  v00042/
    MANIFEST.json                  bundle-format version; data-plugin hash; declared bounds
                                   ...
                                   key, shard, epoch — §4.5); slice and partition lists;
...
**Projection happens once per session, not once per request.** The fragment is projected into row space and cached as a `RowProjection`, and I1's composition is then evaluated as row-space diffs with `∩ base` and `∖ base` clamps. Those clamps are what stop a deny on an entity the fragment never held from driving a tile count negative; they have no home in the entity-space-then-project ordering.
...
Fragments are content-addressed, shared across tokens, and served as immutable frozen views, so "OR the flushed contribution into live masks" is impossible as stated. The mechanism is epoch advance:
...
- Refresh is **lazy and per fragment**: on next use past a staleness threshold, or when `L`'s cost bound trips, the engine builds epoch *n+1* as `fragment@n ∪ delta-postings(satisfied terms, epochs n+1…)`. One OR per delta per live fragment, amortised by laziness so a flush never stampedes the cache. The old epoch stays frozen for draining pins.
- **The effective watermark is always the fragment's own.** A request composes against the watermark of the epoch it actually loaded, so an entity between that watermark and the store's latest is in `L` and directly evaluated — never in neither set. This holds for pinned requests too: the watermark recorded in a pin is advisory, and composition never reads it.
...
Retiring a deny entry on **any** earlier trigger — segment durability, compaction of the row tombstone, WAL retirement — reopens visibility for tokens still on old epochs. That is the fail-open this section exists to forbid, and each of those triggers is individually plausible, which is why the rule is stated as a prohibition on all three rather than as a positive condition alone.

> **⊘ Specified, not implemented.** No deny entry retires. Deletion is a terminal flag; there is no epoch ledger, no retirement floor and no compaction fold. That is fail-*closed* — an entry that never retires never re-exposes anything — but it is not the mechanism described, and the argument above is the only thing carrying the rule until the ledger exists. A suppression, separately, is **non-retirable while active by construction**: no fragment rebuild ever excludes a suppressed entity, so its invisibility rests on the overlay entry for as long as the suppression stands, and assigning it a retirement epoch — as deletion denies have — would eventually expire the entry and make the item visible again.
...
- **In-flight requests drain against their pinned triple**, and the pin manager retains retired prefixes until drained (I11). A drain entry must **not** be an `Arc<Generation>`: holding a whole generation retains the superseded overlay and buffer, which is the R-open failure — a mask that is not stale-restrictive but simply wrong. Reclaim order matters too: verify-then-remove is a use-after-free; remove-then-verify is the fix, and it costs nothing.
...
**Identity-key operations are the exception to "expensive, not risky".** The key is named explicitly on the command line, has no default search path and no environment variable, and a build given no key source refuses before doing any work. A rotation is refused without an explicit flag, resets the identity epoch, and invalidates every identifier a consumer holds — which is why consumers persist `external_id` and treat `tessera_id` as valid only for the epoch it was issued under. A stale identifier presented with its epoch gets a `409`; presented without one, it may silently name a different item, which is the caller's accepted trade.
...
ingest_max_batch_bytes = 16_777_216
...
> **⊘ Specified, not implemented.** There is no metrics emitter: no Prometheus dependency, no `/metrics`, no metrics listener. `/control/status` is the substitute, and it is a pull-only JSON snapshot on the admin plane rather than a time series — so *rates*, including the fragmentation trend the paragraph above exists to catch, must be derived by whatever scrapes it. The fragmentation figure itself is not among the fields listed above.
...
15. **Watermark patching is lazy epoch advance, never in-place mutation.** Correctness comes from I1's live-set composition; patching is an amortised cost optimisation.
