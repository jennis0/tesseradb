...
| [`python-sdk.md`](python-sdk.md) | **Provisional — under review** | The `tesseradb` package's verbs: a database is a directory; `create`, `stage`, `declare_*`, `check`, `commit`, with the first commit a build and every later one ingest (decision 0091 as an API); the local instance as a child process; `map()` and `viewer(terms)` over the existing widget; hosted read only. Six rulings in §11; nothing built |
