...
| [`client-components.md`](client-components.md) | Provisional r3 | The client stack organised by four customers: the wire as C3's product, the headless store as C2's, the `<tessera-explorer>` composite and its pieces as C1's, the demo and the anywidget as C4's — six packages, two count types, a token-and-parts-and-slots styling contract, token-as-message custody. **Nothing in it is built**; §9 is the order. r2 reviewed; r3 re-cut on owner direction and not yet re-reviewed |
