...
| [`sharding.md`](sharding.md) | Provisional | Epoch shards in one process: the entity space and the row space split per shard, so `RowId` stays `u32` past the allocator's ceiling. A shard is a point shard or the one artifact shard, and passes through open, sealed, reopened and dropped. **Nothing in it is built**; seven owner rulings and a staged delivery, S1 through S7 |
