...
| [`artifact-fetch-protocol.md`](artifact-fetch-protocol.md) | **Proposal** | How a client asks for artifacts: the scope and filter axes, the three filter modes (one built, one proposed, one deferred), the cache claim borrowed from the point path, and what the client, the server and an API user each owe — **§4 and §5 are not built** |
...
| [`selection-operand.md`](selection-operand.md) | Provisional | The drawn region as a filter leaf: a box or a lasso decomposed against the Morton cells, so cells wholly inside the shape are whole row ranges and only the cells the boundary crosses take a per-point test — cost linear in the shape's perimeter and independent of its area. The answer is exact for the shape against the stored geometry, with a marked cover fallback past a published perimeter budget. **Nothing is built**; §10 carries three owner rulings, the first being the space the result is expressed in |
