...
| [`sharding.md`](sharding.md) | Provisional | Epoch shards in one process: the entity space and the row space split per shard, so `RowId` stays `u32` past the allocator's ceiling. A shard is a point shard or an artifact shard, and passes through open, sealed and dropped; a freed slot anywhere is reused before a shard opens. **Nothing in it is built**; nine owner rulings and a staged delivery, S1 through S7 |
