...
| A13 | **§5.1's permission for deny dispositions to share the commit window is exercised negatively, and §1.3's r4 parenthetical reads as though it is not.** The window carries ingest only. The argument for declining is at `tessera_lifecycle::window`'s module doc: the saving is one fsync, for the deny alone and only while ingest is concurrent (measured zero on a quiescent node); the ingest path's fsync count per row is unchanged; and the price is a per-entry durability fold over operations §4 scopes differently, an intra-window ordering hazard that does not otherwise exist, and a `WindowEntry` type that stops saying what is true. §1.3's guarantee is "never starved beyond one window" for a different reason than r4 gives — the executor yields at every window close, not because a deny is in the window | A corpus reader meets a permission and a premise, and neither says the system declines them. Candidate for the **⊘ specified, not implemented** marker at §5.1's mixed-window paragraph — which would be its first use, so introducing the convention is an owner call, not a worker's |
