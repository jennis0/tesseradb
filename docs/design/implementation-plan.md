...
Build, in order: the tiler in Rust (Morton sort, row-ID assignment, Arrow segment write, manifest with per-tile row-ID ranges, sidecar column split — one implementation, exercised by `tessera build` now and shared with streaming ingest later); the entity-ID allocator **with signature-sorted assignment within batches from day one** — §11.1's ordering is measured at 8.9–36.7× posting compression and up to 130× on union cost (probes, results §2, §4.2, §4.4), and I9 makes assignment permanent, so shipping created-order in the skeleton locks in an uncompressed index until a full rebuild; the write-ahead log and its ack contract, which belong in the walking skeleton because durability semantics are load-bearing for deletions (system architecture §6.2); the Rust segment loader with mmap and frozen-view mask loading; the term index and mask build; the viewport query resolving a tile to a contiguous row-ID range, intersecting with the mask, and gathering; the `tessera_id` keyed permutation that carries I10 at the boundary *(amended: this read "the opaque per-session handle allocator (I10)", retired from the viewer plane by contracts r6 deviation 8 and design r21 — handles survive only for Phase 3's node handles)*; and a frontend fetching tiles from the service, built on whichever profile applies (see the visualisation architecture).
...
**Note what that package actually delivers.** Flush is not a compaction convenience here — it is the mechanism by which an ingested item becomes visible at all (design §11.2, SA §6.4, lifecycle §5.1). Phase 1's buffered items are durable and carry authorisation state but have no row, so they contribute to no viewport, no count and no density, and their acknowledgement is a durability receipt rather than a visibility promise. Phase 2 is where ingest becomes queryable.

**One precondition and two scope additions, from the 2026-07-30 ingest audit** (design r23, SA r5, contracts r8, lifecycle r4):

1. **Precondition — the side-manifest reader must fail closed before any writer emits deltas.** `SegmentsManifest` parses `deltas`, `tombstones` and `deny` and the read path consults none of them. That is inert while nothing writes them, and it is fail-*open* the moment something does: a manifest carrying tombstones served by a reader that ignores them serves deleted items. Either teach the reader to honour all three, or make it refuse to open a manifest carrying non-empty values it cannot honour — but land that guard **before** the flush writer, not alongside it, so the writer can never outrun the reader.
2. **Scope — backpressure.** `overlay_soft_limit`, `flush_max_items` and `flush_max_age` are design values with no counterpart in config or the engine. Without a fold, deletions are terminal and evaluate entries immortal (lifecycle §3.4), so the overlay and buffer grow monotonically; Phase 1 gets away with it because it has no long-lived ingest, and Phase 2 does not. Memory is already the binding constraint at 10⁹ (the Phase 1 build was OOM-killed at 46.4 GB RSS).
3. **Scope — group-commit allocation** (lifecycle §5.1), now that the owner has confirmed (2026-07-30) that design §3's seconds-to-minutes write budget covers the whole write path, deny dispositions included. Requests are held in a commit window, the whole window is signature-sorted and allocated together, fsync'd once and acknowledged together — which makes design §11.1's sort scope the window rather than the request, **at the server rather than by client convention**, and is the difference between collecting that section's measured compression and forfeiting it to whatever chunk size a client happens to pick. It belongs in this phase because it changes how the flush allocates and is far cheaper to design with the flush than to retrofit. Two rules are not negotiable and should be tested, not assumed: a deny's acknowledgement stays coupled to its application (never a 200 for an entry not yet in force), and changes are still never load-shed. The one genuinely new piece of design is **idempotency across a held window** — contracts §3.4's batch-id check distinguishes *accepted* from *unknown*, and a window adds *held but not yet acknowledged*, which a retry can land in and must join rather than re-allocate.

Land the fragmentation figure on `/control/status` (contracts §3.4) in the same phase, so group commit's effect is measurable as a before-and-after rather than argued.
