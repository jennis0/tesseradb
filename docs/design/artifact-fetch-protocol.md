...
and §8's encodings are on the wire (contracts r44) and §6's fetch model is the shipped channel's
policy; §7.3's server memo arrived from the other direction as the shapeperf track's per-principal
derived cache, so nothing here is ⊘. Promotion is the owner's. A conflict with
...
`artifact_rows: "full" | "identity"` on `/v1/viewport`, defaulting to `"full"` (contracts r44).
...
### 5.3 The rung column — built 2026-08-28 (contracts r44)
...
- **A hold carries what the map draws at rest.** The channel narrows every artifact request —
  per view and whole-scope alike — to `centroid` and `box` (`computed`, contracts r42), and the
  one hull that draws is fetched by identifier when it is needed, per principal. So a held scope
  never carries a ring per artifact, and the hull columns are absent from its frames.
...
- **Defaults carry the caller that holds nothing.** The behaviour every client would need is a
  ...
  client. **The derived-geometry memo is built** — `tessera_engine::derived_cache`, the shapeperf
  track (2026-08-28, `client-delivery.md` S13): one artifact's derived content held under the
  principal's token and every term that could move the visible set or the membership, never shared
  across principals, 64 MiB and evicted least recently used. Measured there: a hover on the 2.42M
  corpus root 70 ms cold and 0.48 ms warm; a pan of twelve viewports 220 ms out and 27 ms back. It
  makes repeat per-view requests cheap for a caller that caches nothing, with no wire field, no
  client obligation and no register pass — the naive client's insurance, which this design never
  depended on and now has.
...
- **The projection is the one wire affordance** (§5.2), and the rung column moves the one
  client-side trap into the wire (§5.3). Both built; contracts r44.
