...
**Date:** 2026-08-30
**Status:** Normative (r13) — **a group grows while the service runs** (r13, 2026-08-31):
...
§4's markers say what remains inside what is built. **the roster is served** (r12, 2026-08-31):
`/v1/meta` publishes every view of every group with its key, ordinal and typed metadata, in
ordinal order, beside the groups' own orderings; every one of them answers a viewer verb, by key
or by ordinal (contracts §3.2 r54). **the permutation is paged** (r11, 2026-08-31): spec §8's
...
> (spec §3.1), the gate (spec §6), and every *serving* surface a scoped column or a scoped layer
> would need — a scoped attribute is stored and is on no filter surface, which spec §5's marker
> states. Each refuses or is absent by name.
...
- **r13 (2026-08-31)** — the write half is built, and the markers at spec §1, §3.2, §3.4 and §4
