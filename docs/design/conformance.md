...
alone in it. The §0 count marker moves with the suite, to **707** — six of them these cases, and **nine** the marker was already behind by: the suite grew after r18 measured it and the marker did not move, which is what a hand-maintained count does. Measured here rather than adjusted: 707 collected, 700 passing, 5 skipped and the same 2 `test_text_differential.py` read timeouts r18 recorded, which fail the same way on `main` under the same load. r18 stands otherwise — **the gate is driven against an oracle** (r18, 2026-08-31; `views.md` §6):
