...
**Status:** Draft r20 — **the highlight, the `member_of` leaf and the browse verb are driven**
(r20, 2026-09-02; `highlight-and-hierarchy.md` §2–§4). Seventeen cases in three modules; the
checked marker moves to **724**. No row of §4.6 is added and none moves position — the evidence
lands in the existing I2, I3 and I12 cells, all three surfaces being the filter contract's own
machinery asked a second question. §0 carries what each module holds. r19 stands otherwise —
**the render-only scoped family is driven against an oracle** (r19,
