...
| | Designed | Built |
|---|---|---|
| Directory layout | `conformance/{fixtures,differential,invariants,lifecycle,oracles,compile-fail}` | one flat `conformance/tests/`, **seventeen** pytest modules, plus `conftest.py` — and `conformance/suite/`'s nine beside them. None of the six designed directories exists in any form *(count corrected 2026-08-30; it read seven, which was true when written and had not moved with the suite)* |
...
| The suite running green | the whole point | **432 of 432** *(r15, 2026-08-20)*. It ran zero for the weeks the harness spawned `tessera serve -c` against a binary taking `--deployment`, then 405 of 432 for the day between that fix and this one. ⊘ `conformance/suite` — the correctness suite's shared battery, not this matrix — needs Python 3.11+ for `tomllib` and was not run |
...
**Six covered as designed, one covered in the form written for it, one in substance, one incidental, five not covered — one of those five for want of an implementation rather than for want of a test** *(r14; six were not covered, and I3 moved because a test was written. r13: three of the six were so recorded before the annotation machinery was built, and I5 and I13b sit between the two, each needing a second partition or a divergent plugin before any oracle could disagree)*. The plan calls the suite the deliverable; this is where it stands.
