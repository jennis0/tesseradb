...
| **Text scan** | **3.5 ns `eq` / 4.5 `prefix` / 8.3 `in` / 1.7 `contains`**, contiguous; 28–96 ns scattered | measured at 10⁸ (arms 6, 12). ~14× a category on the per-value predicates, because a value streams ~22 bytes against 4 — equality is at memory bandwidth for the shape. `contains` is *below* them because it searches the run's concatenated bytes rather than each value |
...
| **An unselective predicate** | **3.4 s** matching 25% of a whole-corpus candidate, **6.0 s** at 50% | measured at 10⁹ (arm 7). Decomposes ~40% mispredicted branches / ~30% Roaring / ~30% buffer traffic, so **no accelerator over the column touches it** — the largest known gap in the filter path |
| Branchless collection | **Refuted as a fix.** Flat ~445 ms across all selectivities *in isolation* (10× at its worst), but 1.5–2.1× worse on selective arms for 1.3–1.6× here | measured both ways at 10⁹ (arm 7); attractive only alongside a cheaper result representation |
