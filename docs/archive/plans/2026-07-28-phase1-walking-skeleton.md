> **ARCHIVED 2026-08-01 — EXECUTED. Tasks 0–15 complete; Task 16 paused at the 10⁹ OOM and its remainder folded into the external-id plan. The unticked boxes below were never the record — completion was tracked in the SDD ledger.**
>
...
- [ ] **Step 5: Write `docs/archive/plans/phase1-results.md`** — every number from steps 2–4 against each plan-§5 exit criterion, pass/fail, plus any deviations taken. Commit everything. `git commit -am "test(bench): 10^9 exit-criteria measurement"`
