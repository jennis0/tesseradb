> **ARCHIVED 2026-08-01 — PARTIALLY EXECUTED. Tasks 1–8 ran; Task 10 (citations and publish) never did, which is why the paper shipped with an empty references section. The paper is archived alongside this plan and will be regenerated from docs/design/, not repaired.**
>
...
| Path | Responsibility |
|---|---|
| `docs/archive/whitepaper/build.py` | Concatenates `src/` fragments in order into the published file |
| `docs/archive/whitepaper/validate.py` | Static checks: CSP compliance, no document tags, figure IDs present, theme rules present |
| `docs/archive/whitepaper/render_check.py` | Playwright harness: renders built page at 1280px and 390px, light and dark; fails on console errors or horizontal body scroll; writes screenshots |
| `docs/archive/whitepaper/src/00-style.html` | The entire design system: tokens, type scale, layout, figure chrome, theme overrides |
| `docs/archive/whitepaper/src/01-header.html` | `<title>`, title block, standfirst, the status note |
| `docs/archive/whitepaper/src/1x-part-*.html` | One fragment per part (six), prose plus its figures |
| `docs/archive/whitepaper/src/90-footer.html` | Citations block, closing note |
| `docs/archive/whitepaper/src/lib/figure.js` | Shared figure runtime: theme observer, SVG helpers, number formatting |
| `docs/archive/whitepaper/facts/part-*.md` | Cited fact sheets from Task 1 (claim → document → section) |
...
**Files:**
- Create: `docs/archive/whitepaper/facts/part-1.md` … `part-6.md`
...
```bash
mkdir -p /home/joe/code/tessera/docs/archive/whitepaper/facts
```
...
**Files:**
- Create: `docs/archive/whitepaper/build.py`, `docs/archive/whitepaper/validate.py`, `docs/archive/whitepaper/render_check.py`
- Create: `docs/archive/whitepaper/src/00-style.html`, `docs/archive/whitepaper/src/01-header.html`, `docs/archive/whitepaper/src/90-footer.html`, `docs/archive/whitepaper/src/lib/figure.js`

**Interfaces:**
- Produces: `python3 docs/archive/whitepaper/build.py` → writes `docs/tessera-white-paper.html`. `python3 docs/archive/whitepaper/validate.py` → exit 0 on pass, prints one line per failure. `python3 docs/archive/whitepaper/render_check.py` → exit 0 on pass, writes screenshots to the scratchpad.
...
- Produces: `docs/archive/whitepaper/src/lib/figure.js` exporting `onThemeChange(fn)`, `svg(tag, attrs)`, and `fmt(n)`.
...
```bash
cd /home/joe/code/tessera && python3 docs/archive/whitepaper/build.py
```
...
Create `docs/archive/whitepaper/src/00-style.html` containing a single `<style>` block that defines:
...
Create `docs/archive/whitepaper/src/lib/figure.js`. It is not a `.html` fragment, so the build script will not pick it up — inline its contents inside a `<script>` block at the end of `00-style.html`. It exports, on a global `TF` object: `onThemeChange(fn)` (a `matchMedia` listener plus a `MutationObserver` on `documentElement`'s `data-theme`), `svg(tag, attrs)` (namespaced element factory), and `fmt(n)` (thousands separators, and `µs`/`ms` suffixing).

- [ ] **Step 10: Build and verify the scaffolding passes its own checks**

```bash
cd /home/joe/code/tessera
python3 docs/archive/whitepaper/build.py
python3 docs/archive/whitepaper/validate.py
...
**Files:**
- Create: `docs/archive/whitepaper/src/20-part-2.html`
- Read: `docs/archive/whitepaper/facts/part-2.md`
...
```bash
cd /home/joe/code/tessera && python3 docs/archive/whitepaper/validate.py | grep -E "f3|f4|f5"
```
...
**Files:**
- Create: `docs/archive/whitepaper/src/30-part-3.html`
- Read: `docs/archive/whitepaper/facts/part-3.md`
...
```bash
cd /home/joe/code/tessera && python3 docs/archive/whitepaper/validate.py | grep -E "f6|f7|f8"
```
...
**Files:**
- Create: `docs/archive/whitepaper/src/40-part-4.html`
- Read: `docs/archive/whitepaper/facts/part-4.md`
...
```bash
cd /home/joe/code/tessera && python3 docs/archive/whitepaper/validate.py | grep -E "f9|f10|f11|f12"
```
...
**Files:**
- Create: `docs/archive/whitepaper/src/50-part-5.html`
- Read: `docs/archive/whitepaper/facts/part-5.md`
...
```bash
cd /home/joe/code/tessera && python3 docs/archive/whitepaper/validate.py | grep -E "f13|f14"
```
...
**Files:**
- Create: `docs/archive/whitepaper/src/60-part-6.html`
- Read: `docs/archive/whitepaper/facts/part-6.md`
...
```bash
cd /home/joe/code/tessera && python3 docs/archive/whitepaper/validate.py | grep -E "f15|f16"
```
...
**Files:**
- Modify: whichever `docs/archive/whitepaper/src/*.html` fragments the review identifies
...
**Files:**
- Modify: `docs/archive/whitepaper/src/00-style.html`, `docs/archive/whitepaper/src/90-footer.html`
