...
| Path | What it is |
|---|---|
| `docs/archive/whitepaper/build.py` | Concatenates `src/*.html` (sorted) into `docs/tessera-white-paper.html`. Transcribed verbatim from the brief. |
| `docs/archive/whitepaper/validate.py` | Static checks: document-level tags, external resource loads, `<link>`, network APIs, `<title>`, the seventeen figure IDs, the three theme rules. Transcribed verbatim. |
| `docs/archive/whitepaper/render_check.py` | Playwright harness: wraps the fragment in a document, loads it at 1280/390 × light/dark, fails on console errors or body horizontal overflow, writes four full-page screenshots. Transcribed verbatim. |
| `docs/archive/whitepaper/src/00-style.html` | The design system: one `<style>` block (tokens, layout, type, figure chrome, tables, focus, motion, responsive) plus one `<script>` block holding an inline copy of the figure runtime. |
| `docs/archive/whitepaper/src/01-header.html` | `<title>`, opens `<article class="wp">`, eyebrow, `<h1>`, standfirst, status note. |
| `docs/archive/whitepaper/src/90-footer.html` | Empty `<section id="citations">` for Task 10, closes `</article>`. |
| `docs/archive/whitepaper/src/lib/figure.js` | Reference copy of the figure runtime. Not globbed by the build; kept byte-identical to the inline copy in `00-style.html`. |
...
python3 -c "
import re,pathlib
js=pathlib.Path('docs/archive/whitepaper/src/lib/figure.js').read_text().rstrip()
inl=re.search(r'<script>\n(.*)\n</script>',pathlib.Path('docs/archive/whitepaper/src/00-style.html').read_text(),re.S).group(1)
print(js==inl)"
```
...
$ python3 docs/archive/whitepaper/validate.py ; echo exit=$?
...
$ <venv>/bin/python docs/archive/whitepaper/render_check.py ; echo exit=$?
PASS
exit=0
```
