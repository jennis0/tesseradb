 ...
 * (decision 0104; contracts §3.2 r44).
 *
...
export class ArtifactChannel {
   ...
   * others (contracts §3.2 r44). Returning the held object where the bit agrees is what keeps a re-served
    ...
    try {
      const response = await this.client.viewport(
        token,
        {
          ...
          // The same narrowing the per-view request sends: a hold carries what the map draws at
          // rest — centroid and box — and the one hull that draws is fetched by identifier
          // (`needHull`), so a whole-scope fetch never derives a ring per artifact.
