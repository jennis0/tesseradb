...
*Where it does belong, and one constraint that bites there.* The client, by choosing the **depth it requests independently of the viewport's zoom** — client-interaction §10's global point budget, which that document names, attributes to Potree and CesiumJS, and records as unwritten. Because priority prefixes nest, requesting depth *d* at a shallow zoom is a superset of the natural tile and pops nothing. The arithmetic is `marks ≈ m_target · 4^d`, so a 5 × 10⁴ budget at `m_target = 16` wants **depth 6 — 4,096 tiles** over a full-extent view. That runs straight into **`serve.max_tiles_per_request`**, an availability guard sized for a viewport's worth of tiles rather than a budget's worth, and the two numbers have never been reconciled. Recorded as the constraint the budget work must resolve; not settled here, because it is a serving-availability question rather than a selection one, and because `m_target` remains provisional pending the perceptual measurement this section already calls for.
