...
**Status:** Draft for review — revision 22
...
**The definition.** Every item carries a fixed pseudo-random **priority**, derived as the **high 16 bits of the item's `tessera_id`** (contracts §2.6), which is a keyed permutation of `(shard_id, entity_id)` *(r21; previously "derived by hashing its entity ID" — an unkeyed `splitmix64`)*. A tile's sample is the *k* **lowest-priority items in that tile inside the governing mask**, where *k* is the on-screen mark budget per tile. Nothing in that definition references unmasked data, so it satisfies I7 by construction, and it nests provably: an item among the *k* lowest-priority visible items in a parent tile is necessarily among the *k* lowest in whichever child contains it, because the child's visible set is a subset. The nesting argument depends only on priority being a fixed per-item constant and mask-independent, which it remains.
...
Owner and review date for C1, C4, C6, C12, C14, C15, C16 and C17 to be assigned before launch.
...
- **r22** — Two corrections owed to r21, both raised by one question: *now that `tessera_id` leaks nothing about entity density, is the permutation still needed?* The answer is yes, and neither correction changes a mechanism, a format or an invariant. **§5.1's inverse-direction clause was stale**: it said the row→entity direction "is the entity-ID column of the row-ordered store", naming an artifact contracts r6 removed — `columns.arrow` carries `tessera_id`, and row→entity is the inverse of the keyed bijection, a pure function needing no file. The policy never changed (the inverse permutation is not stored); only the sentence describing where it comes from. Appendix A was already correct at r21 and is untouched. **§11.1's Morton-order prohibition was under-argued**: it rested solely on C6, which r21 moved to *Accepted — caller's control*, leaving a live prohibition standing on a retired argument. It is restated on the two grounds that were always stronger and are not disclosure arguments — Morton rank is not permanent, so a Morton-derived entity ID violates **I9** at the first append; and the batch ordering is already spent on the term-signature grouping that is the entire measured posting compression (r18) — with an explicit note that a future reader must not reinstate the alternative on the grounds that C6 has been relaxed. §5.1 additionally records, as the standing answer to the recurring question, that only `entity_to_row` is stored and why it is irreducible (permanence against churn; the spent ordering; one entity-space index across independently-ranked slices and partitions), that it is not derivable the way the inverse now is because it is a function of geometry, and that what is genuinely open is the array's **encoding** rather than its existence (implementation plan §14). Companion annotation in the contracts spec's §0.3 deviation 2, which carried the same stale mechanism under a supersession deviation 6 already recorded.
