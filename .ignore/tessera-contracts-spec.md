...
`POST /v1/viewport` — `{slice, zoom, bbox: [x0,y0,x1,y1], k?, filters?, pin?, underlay_offset?}`; `k` defaults to 30, capped by `max_k`. Response **Arrow**, two batches, or three when the underlay is requested:
